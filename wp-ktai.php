...
// 出力のバッファリングを開始し、コールバック関数として"mb_outputHandler"
// を指定する
...
if (file_exists(__DIR__ . '/xoops_version.php')) {
    require dirname(__DIR__, 2) . '/mainfile.php'; //XOOPSの場合には呼び出されるはず。
}
...
if (false !== strpos($HTTP_USER_AGENT, "DoCoMo")) {
    $ua_list = explode('/', $HTTP_USER_AGENT);

    $is_docomo = mb_substr($ua_list[3], 1);
...
function get_post_titles($start, $count)
{
    ...
    $titles = $wpdb->get_results(
        'SELECT ID,post_title,post_date ' . "FROM $tableposts WHERE " . "post_status = 'publish' " . 'ORDER BY post_date DESC ' . "LIMIT $start,$count"
    );
...
function get_postdata_num($num)
{
    ...
    $post = $wpdb->get_row(
        'SELECT ID ' . "FROM $tableposts WHERE " . "post_status = 'publish' " . 'ORDER BY post_date DESC ' . "LIMIT $num,1"
    );
...
$ackeychar = ['&#63888;', '&#63879;', '&#63880;', '&#63881;', '&#63882;', '&#63883;', '&#63884;', '&#63885;', '&#63886;', '&#63887;']; //i-modeのキー番号の文字
...
switch ($_REQUEST['view']) {
    case 'image':
        ...
        if (eregi('.jpeg', $orgimg) || eregi('.jpg', $orgimg) || IMAGETYPE_JPEG == $imginfo[2]) {
        ...
        } elseif (eregi('.gif', $orgimg) || IMAGETYPE_GIF == $imginfo[2]) {
...
switch ($_REQUEST['view']) {
    case 'list':
        ...
        foreach ($title_lists as $title) {
            ...
            $tmp = mb_substr($title->post_date, 5, 2) . '/' . mb_substr($title->post_date, 8, 2) . mb_substr($title->post_date, 10, 6);
    ...
    case 'content':
        ...
        $tmp = mb_substr($post['Date'], 5, 2) . '/' . mb_substr($post['Date'], 8, 2) . mb_substr($post['Date'], 10, 6);
        $echostring .= $post['Title'] . '(' . $tmp . ') Page:' . $nextpage . '<hr>';
        $pages[0] = $post['Content'];
        $page = 1;
        $more = 1;
        ...
        if ($CharCountPerPage > 0) {
            ...
            if (preg_match('/<[^>]*?$/', $tmp1, $match)) {
                $nextstart -= mb_strlen($match[0]);

                $prevstart -= mb_strlen($match[0]);
            ...
            $nextpagelen = mb_strlen(mb_strcut($tmp, $nextstart));
    ...
    case 'comprev':
        ...
        if ($comments) {
            foreach ($comments as $comment) {
                ...
                $tmp = mb_substr($comment->comment_date, 5, 2) . '/' . mb_substr($comment->comment_date, 8, 2) . mb_substr($comment->comment_date, 10, 6);
