...
if (file_exists(__DIR__ . '/wp-mail-conf.php')) {
    @require_once __DIR__ . '/wp-mail-conf.php';
...
function wp_mail_receive()
{
    ...
    for ($iCount = 1; $iCount <= $Count; $iCount++) {
        ...
        if ((!$MsgOne) || ('array' != gettype($MsgOne))) {
        ...
        while (list($lineNum, $line) = each($MsgOne)) {
            ...
            if ($bodysignal) {
                ...
                if (preg_match('/^Content-Type:\s+(.*?)\;/i', $line, $match)) {
                    ...
                    $content_type = mb_strtolower($match[1]);
                }

                if (('multipart/mixed' == $content_type) && (preg_match('/boundary=(?:")?([^;"\s\n]*?)(?:")?\s*(?:$|;)/', $line, $match)) && ('' == $att_boundary)) {
                    $att_boundary = trim($match[1]);
                }

                if (('multipart/alternative' == $content_type) && (preg_match('/boundary=(?:")?([^;"\s\n]*?)(?:")?\s*(?:$|;)/', $line, $match)) && ('' == $boundary)) {
                    $boundary = trim($match[1]);
                }

                if (('multipart/related' == $content_type) && (preg_match('/boundary=(?:")?([^;"\s\n]*?)(?:")?\s*(?:$|;)/', $line, $match)) && ('' == $hatt_boundary)) {
                ...
                if (preg_match('/Subject: /', $line)) {
                    ...
                    $subject = mb_substr($subject, 9, mb_strlen($subject) - 9);
                ...
                if (preg_match('/Date: /', $line)) { // of the form '20 Mar 2002 20:32:37'
                    ...
                    if (mb_strpos($ddate, ',')) {
                        $ddate = trim(mb_substr($ddate, mb_strpos($ddate, ',') + 1, mb_strlen($ddate)));
                    }
                    ...
                    $mail_timezone = trim(preg_replace("\([^)]*\)", '', $date_arr[4])) / 100;
                    ...
                    $ddate_U += ($mai_time_difference * 3600);
        ...
        if (preg_match('/' . get_settings('subjectprefix') . '/', $subject)) {
            ...
            if (('' == $charset) || ('ISO-2022-JP' == trim(mb_strtoupper($charset)))) {
                $charset = 'JIS';
            }

            if ('SHIFT_JIS' == trim(mb_strtoupper($charset))) {
                $charset = 'SJIS';
            }

            $btpos = mb_strpos($content, get_settings('bodyterminator'));

            if ($btpos) {
                $content = mb_substr($content, 0, $btpos);
            }
            ...
            if (get_settings('use_phoneemail')) {
                ...
                $btpos = mb_strpos($firstline, get_settings('phoneemail_separator'));

                if ($btpos) {
                    $userpassstring = trim(mb_substr($firstline, 0, $btpos));

                    $content = trim(mb_substr($content, $btpos + mb_strlen(get_settings('phoneemail_separator')), mb_strlen($content)));

                    $btpos = mb_strpos($content, get_settings('phoneemail_separator'));

                    if ($btpos) {
                        $userpassstring = trim(mb_substr($content, 0, $btpos));
            ...
            if (0 == strncmp($secondlineParts[0], 'POS', 3)) {
                echo "Found POS:<br>\n";
                ...
                $flat = (float)$secLineParts[0];

                $flon = (float)$secLineParts[1];
            ...
            if ($user_level > 0) {
                ...
                if (!get_settings('emailtestonly')) {
                    ...
                    if (('' != $boundary) && ('' != $hatt_boundary)) {
                        for ($i = 2, $iMax = count($contents); $i < $iMax; $i++) {
...
function wp_getattach($content, $prefix = '', $create_thumbs = 0)
{
    ...
    if ($subtype) {
        $subtype = mb_strtolower($matches[1]);
    }

    $temp_file = '';

    if (in_array($subtype, ['gif', 'jpg', 'jpeg', 'png'], true)) {
        if ((0 == $prefix) && eregi("name=\"?([^\"\n]+)\"?", $contents[0], $filereg)) {
        ...
        if (eregi('Content-Transfer-Encoding:.*base64', $contents[0])) {
            $tmp = base64_decode($contents[1], true);
    ...
    if (file_exists($file)) {
        ...
        if (!function_exists('imagegif') && 1 == $type[2]) {
            ...
            if (function_exists('gd_info')) {
                ...
                if (false !== mb_strstr($gdver['GD Version'], '1.')) {
                    //For GD
            ...
            $thumbpath = mb_substr($file, 0, mb_strrpos($file, '/')) . '/thumb-' . $path[count($path) - 1];
