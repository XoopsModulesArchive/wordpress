...
if (!defined('WP_CATEGORIES_INCLUDED')) {
    ...
    function b_wp_categories_show($options, $wp_num = '')
    {
        ...
        $with_count = ($options[1]) ?: 0;

        $sorting_key = ($options[2]) ?: 'name';

        $sorting_order = ($options[3]) ?: 'asc';
        ...
        if (preg_match('/^' . preg_quote(XOOPS_ROOT_PATH . '/modules/wordpress' . $wp_num . '/', '/') . '/i', $cur_PATH)) {
            $cat = array_key_exists('cat', $_GET) ? (int)$_GET['cat'] : null;

            $category_name = array_key_exists('category_name', $_GET) ? $_GET['category_name'] : '';

            if ($category_name and 0 == $cat) {
        ...
        if (0 == $block_style) {
            ...
            $block['content'] = preg_replace('\<select name\=[^\>]*\>', $select_str, $block_str);
        }
