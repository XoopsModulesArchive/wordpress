...
if (!defined('WP_RECENT_COMMENTS_INCLUDED')) {
    ...
    {
        $block_style = ($options[0]) ?: 0;

        $num_of_list = $options[1] ?? 10;

        $show_rss_icon = $options[2] ?? 0;

        $cat_date = $options[3] ?? ($block_style ? 1 : 0);

        $show_type = $options[4] ?? 1;
