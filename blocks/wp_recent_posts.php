...
if (!defined('WP_RECENT_POSTS_INCLUDED')) {
    ...
    function b_wp_recent_posts_edit($options, $wp_num = '')
    {
        ...
        $form .= preg_replace('\<select name\=[^\>]*\>', $select_str, $list_str);

        $form .= '</td></tr>';

        $form .= '<tr><td>Display New Flag:</td>';

        $chk = (1 == $options[8]) ? ' checked' : '';

        $form .= "<td><input type='radio' name='options[8]' value='1'" . $chk . '>&nbsp;' . _YES . '&nbsp;';

        $chk = (0 == $options[8]) ? ' checked' : '';
    ...
    function b_wp_recent_posts_show($options, $wp_num = '')
    {
        ...
        if ((empty($category)) || ('all' == $category) || ('0' == $category)) {
            $whichcat = '';
        ...
        if ($lposts) {
            ...
            foreach ($lposts as $lpost) {
                ...
                if ($new_flg) {
                    ...
                    $elapse = time() + get_settings('time_difference') * 3600 - mktime(mb_substr($m, 11, 2), mb_substr($m, 14, 2), mb_substr($m, 17, 2), mb_substr($m, 5, 2), mb_substr($m, 8, 2), mb_substr($m, 0, 4));

                    if ($elapse < 1 * 60 * 60 * 24) {
                        $newstr = ' <span class="new1">New!</span>';
                    } elseif ($elapse < 7 * 60 * 60 * 24) {
