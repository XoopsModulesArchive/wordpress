...
function unregister_GLOBALS()
{
    ...
    $noUnset = ['GLOBALS', '_GET', '_POST', '_COOKIE', '_REQUEST', '_SERVER', '_ENV', '_FILES', 'table_prefix'];
...
if (isset($_SERVER['SCRIPT_FILENAME']) && (mb_strpos($_SERVER['SCRIPT_FILENAME'], 'php.cgi') == mb_strlen($_SERVER['SCRIPT_FILENAME']) - 7)) {
    $_SERVER['SCRIPT_FILENAME'] = $_SERVER['PATH_TRANSLATED'];
}

// Fix for Dreamhost and other PHP as CGI hosts
if (mb_strstr($_SERVER['SCRIPT_NAME'], 'php.cgi')) {
...
require_once ABSPATH . 'include/xoops-wp.php';

$wpdb->hide_errors();
$db_check = $wpdb->get_var("SELECT option_value FROM $wpdb->options WHERE option_name = 'siteurl'");
if (!$db_check && (!mb_strstr($_SERVER['PHP_SELF'], 'install.php') && !defined('WP_INSTALLING'))) {
...
if (!mb_strstr($_SERVER['PHP_SELF'], 'install.php')) :
    ...
    if (is_array($current_plugins)) {
        foreach ($current_plugins as $plugin) {
            if ('' != $plugin && file_exists(ABSPATH . 'wp-content/plugins/' . $plugin)) {
                require_once ABSPATH . 'wp-content/plugins/' . $plugin;
            }
        }
    }
}
