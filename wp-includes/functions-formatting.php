...
function wptexturize($text)
{
    ...
    $textarr = preg_preg_split('/(<.*>)/Us', $text, -1, PREG_SPLIT_DELIM_CAPTURE);
    ...
    for ($i = 0; $i < $stop; $i++) {
        ...
        } elseif (mb_strstr($curl, '<code') || mb_strstr($curl, '<pre') || mb_strstr($curl, '<kbd' || mb_strstr($curl, '<style') || mb_strstr($curl, '<script'))) {
            // strstr is fast
...
function utf8_uri_encode($utf8_string)
{
    ...
    for ($i = 0, $iMax = mb_strlen($utf8_string); $i < $iMax; $i++) {
...
/*
 ...
 @param text $is_comment      Text to be balanced
...
function balanceTags($text, $is_comment = 0)
{
    ...
    while (preg_match("/<(\/?\w*)\s*([^>]*)>/", $text, $regex)) {
        ...
        if ('/' == $regex[1][0]) { // End Tag
            $tag = mb_strtolower(mb_substr($regex[1], 1));
            ...
            $tag = mb_strtolower($regex[1]);

            // Tag Cleaning

            // If self-closing or '', don't do anything.

            if (('/' == mb_substr($regex[2], -1)) || ('' == $tag)) {
            } // ElseIf it's a known single-entity tag but it doesn't close itself, do so

            elseif ('br' == $tag || 'img' == $tag || 'hr' == $tag || 'input' == $tag) {
                $regex[2] .= '/';
            } else {    // Push the tag onto the stack
                // If the top of the stack is the same as the tag we want to push, close previous tag

                if (($stacksize > 0) && ('div' != $tag) && ($tagstack[$stacksize - 1] == $tag)) {
                    $tagqueue = '</' . array_pop($tagstack) . '>';
        ...
        $newtext .= mb_substr($text, 0, $i) . $tag;

        $text = mb_substr($text, $i + $l);
    }
...
function antispambot($emailaddy, $mailto = 0)
{
    ...
    for ($i = 0, $iMax = mb_strlen($emailaddy); $i < $iMax; $i += 1) {
        $j = floor(mt_rand(0, 1 + $mailto));

        if (0 == $j) {
            $emailNOSPAMaddy .= '&#' . ord(mb_substr($emailaddy, $i, 1)) . ';';
        } elseif (1 == $j) {
            $emailNOSPAMaddy .= mb_substr($emailaddy, $i, 1);
        } elseif (2 == $j) {
            $emailNOSPAMaddy .= '%' . zeroise(dechex(ord(mb_substr($emailaddy, $i, 1))), 2);
        }
    }
...
function convert_smilies($text)
{
    ...
    if (get_settings('use_smilies')) {
        ...
        $textarr = preg_preg_split('/(<.*>)/U', $text, -1, PREG_SPLIT_DELIM_CAPTURE); // capture the tags as well as in between
        $stop = count($textarr); // loop stuff
        for ($i = 0; $i < $stop; $i++) {
            $content = $textarr[$i];

            if ((mb_strlen($content) > 0) && ('<' != $content[0])) { // If it's not a tag
...
function iso8601_timezone_to_offset($timezone)
{
    ...
    if ('Z' == $timezone) {
        ...
        $sign = ('+' == mb_substr($timezone, 0, 1)) ? 1 : -1;

        $hours = (int)mb_substr($timezone, 1, 2);

        $minutes = (int)mb_substr($timezone, 3, 4) / 60;
