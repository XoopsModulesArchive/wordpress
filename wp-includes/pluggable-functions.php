...
if (!function_exists('wp_mail')) :
    ...
    {
        if ('' == $headers) {
            $headers = "MIME-Version: 1.0\n" . 'From: wordpress@' . preg_replace('#^www\.#', '', mb_strtolower($_SERVER['SERVER_NAME'])) . "\n" . 'Content-Type: text/plain; charset="' . get_settings('blog_charset') . "\"\n";
        }
...
if (!function_exists('check_admin_referer')) :
    ...
    {
        $adminurl = mb_strtolower(get_settings('siteurl')) . '/wp-admin';

        $referer = mb_strtolower(wp_get_referer());

        if (!wp_verify_nonce($_REQUEST['_wpnonce'], $action)
            && !(-1 == $action && mb_strstr($referer, $adminurl))) {
            wp_nonce_ays($action);
...
if (!function_exists('check_ajax_referer')) :
    function check_ajax_referer()
    {
        ...
        foreach ($cookie as $tasty) {
            if (false !== mb_strpos($tasty, USER_COOKIE)) {
                $user = urldecode(mb_substr(mb_strstr($tasty, '='), 1));
            } // Nasty double encoding

            if (false !== mb_strpos($tasty, PASS_COOKIE)) {
                $pass = urldecode(mb_substr(mb_strstr($tasty, '='), 1));
            }
        }
...
if (!function_exists('wp_get_cookie_login')):
    function wp_get_cookie_login()
    {
        ...
        return ['login' => $_COOKIE[USER_COOKIE], 'password' => $_COOKIE[PASS_COOKIE]];
    }
...
if (!function_exists('wp_setcookie')) :
    function wp_setcookie($username, $password, $already_md5 = false, $home = '', $siteurl = '', $remember = false)
    {
        ...
        } // Double hash the password in the cookie.
...
if (!function_exists('wp_notify_postauthor')) :
    function wp_notify_postauthor($comment_id, $comment_type = '')
    {
        ...
        $wp_email = 'wordpress@' . preg_replace('#^www\.#', '', mb_strtolower($_SERVER['SERVER_NAME']));
...
if (!function_exists('wp_notify_moderator')) :
    function wp_notify_moderator($comment_id)
    {
        ...
        if (0 == get_settings('moderation_notify')) {
            return true;
        }
...
if (!function_exists('wp_verify_nonce')) :
    function wp_verify_nonce($nonce, $action = -1)
    {
        ...
        if (mb_substr(wp_hash($i . $action . $uid), -12, 10) == $nonce || mb_substr(wp_hash(($i - 1) . $action . $uid), -12, 10) == $nonce) {
            return true;
        }
...
if (!function_exists('wp_create_nonce')) :
    function wp_create_nonce($action = -1)
    {
        ...
        return mb_substr(wp_hash($i . $action . $uid), -12, 10);
    }
endif;
