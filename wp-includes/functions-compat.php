...
if (!function_exists('is_a')) {
    function is_a($object, $class)
    {
        ...
        if (get_class($object) == mb_strtolower($class)) {
            return true;
        }
...
function printr($var, $do_not_echo = false)
{
    ...
    $code = htmlentities(ob_get_contents(), ENT_QUOTES | ENT_HTML5);
...
if (!function_exists('array_change_key_case')) {
    function array_change_key_case($input, $case = CASE_LOWER)
    {
        if (!is_array($input)) {
            trigger_error(
                'array_change_key_case(): The argument should be an array',
                E_USER_WARNING
            );
        ...
        $casefunc = (CASE_LOWER == $case) ? 'strtolower' : 'strtoupper';
