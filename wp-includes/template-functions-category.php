...
function dropdown_cats(
    ...
    if ($categories) {
        foreach ($categories as $category) {
            ...
            if (1 == (int)$optioncount) {
                echo '&nbsp;&nbsp;(' . $category->cat_count . ')';
            }

            if (1 == (int)$optiondates) {
...
function list_cats(
    ...
    if (!empty($exclude)) {
        $excats = preg_preg_split('/[\s,]+/', $exclude);

        if (count($excats)) {
            foreach ($excats as $excat) {
                $exclusions .= ' AND cat_ID <> ' . (int)$excat . ' ';
            }
        }
    }
    ...
    if (0 == (int)$categories) {
        ...
        if (empty($optioncount) || empty($GLOBALS['wp_xoops_author']) || !wp_xoops_ismu()):
            $query = "
			SELECT cat_ID, cat_name, category_nicename, category_description, category_parent, category_count
			FROM $wpdb->categories
			WHERE cat_ID > 0 $exclusions
			ORDER BY $sort_column $sort_order"; else:
            $exclusions = preg_replace('/AND cat_ID <>/', 'AND pc.category_id <>', $exclusions);

        $query = "
			SELECT c.cat_ID, c.cat_name, c.category_nicename, c.category_description, c.category_parent, COUNT(*) AS category_count
			FROM 
				$wpdb->post2cat AS pc
				LEFT JOIN $wpdb->posts AS p ON pc.post_id = p.ID
				LEFT JOIN $wpdb->categories AS c ON pc.category_id = c.cat_ID
			WHERE
				pc.category_id > 0 $exclusions
				AND 
				p.post_status = 'publish'
				AND 
				p.post_author = {$GLOBALS['wp_xoops_author']}
    ...
    if ($optiondates) {
        ...
        foreach ($cat_dates as $cat_date) {
            $category_timestamp[(string)$cat_date->category_id] = $cat_date->ts;
        }
    }
    ...
    foreach ((array)$categories as $category) {
        if ((0 == (int)$hide_empty || $category->category_count) && (!$hierarchical || $category->category_parent == $child_of)) {
            $num_found++;

            $link = '<a href="' . get_category_link($category->cat_ID) . '" ';

            if (0 == $use_desc_for_title || empty($category->category_description)) {
            ...
            if (1 == (int)$optioncount) {
                $link .= ' (' . (int)$category->category_count . ')';
            }
            ...
            if ($optiondates) {
                ...
                $link .= ' ' . gmdate($optiondates, $category_timestamp[(string)$category->cat_ID]);
            }
