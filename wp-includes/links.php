 ...
 * @param mixed $after
 * @param mixed $between
 * @param mixed $show_images
 * @param mixed $orderby
 * @param mixed $show_description
 * @param mixed $show_rating
 * @param mixed $limit
 * @param mixed $show_updated
 */
function get_linksbyname(
    $cat_name = 'noname',
    $before = '',
...
function wp_get_linksbyname($category, $args = '')
{
    ...
    $cat = $wpdb->get_row(
        'SELECT cat_id, cat_name, auto_toggle, show_images, show_description, ' . ' show_rating, show_updated, sort_order, sort_desc, text_before_link, text_after_link, ' . " text_after_all, list_limit FROM $wpdb->linkcategories WHERE cat_name='$category'"
    );
...
function wp_get_links($args = '')
{
    ...
    if (!empty($args) && false === mb_strpos($args, '=')) {
        // If args is not a query string, it's a category id.

        $category = $args;

        $cat = $wpdb->get_row(
            'SELECT cat_id, cat_name, auto_toggle, show_images, show_description, ' . ' show_rating, show_updated, sort_order, sort_desc, text_before_link, text_after_link, ' . " text_after_all, list_limit FROM $wpdb->linkcategories WHERE cat_id=$category"
        );
...
function get_links(
    ...
    if ('_' == mb_substr($orderby, 0, 1)) {
        $direction = ' DESC';

        $orderby = mb_substr($orderby, 1);
    }
    ...
    foreach ($results as $row) {
        ...
        if ($show_updated) {
            if ('00' != mb_substr($row->link_updated_f, 0, 2)) {
        ...
        if ((null != $row->link_image) && $show_images) {
            if (mb_strstr($row->link_image, 'http')) {
