...
function bloginfo($show = '')
{
    ...
    if (!mb_strstr($show, 'url')
        && //don't filter URLs
        !mb_strstr($show, 'directory')
        && !mb_strstr($show, 'home')) {
...
function wp_title($sep = '&raquo;', $display = true)
{
    ...
    if (!empty($m)) {
        $my_year = mb_substr($m, 0, 4);

        $my_month = $month[mb_substr($m, 4, 2)];
...
function single_cat_title($prefix = '', $display = true)
{
    ...
    if (!empty($cat) && !('ALL' == mb_strtoupper($cat))) {
...
function get_archives($type = '', $limit = '', $format = 'html', $before = '', $after = '', $show_post_count = false)
{
    ...
    if ('monthly' == $type) {
        $arcresults = $wpdb->get_results(
            "SELECT DISTINCT YEAR(post_date) AS `year`, MONTH(post_date) AS `month`, count(ID) as posts FROM $wpdb->posts WHERE post_date < '$now' AND post_date != '0000-00-00 00:00:00' AND post_status = 'publish' "
            . ((wp_xoops_ismu() && !empty($GLOBALS['wp_xoops_author'])) ? ' AND post_author = '
                                                                          . (int)$GLOBALS['wp_xoops_author'] : '')
        ...
        $arcresults = $wpdb->get_results(
            "SELECT DISTINCT YEAR(post_date) AS `year`, MONTH(post_date) AS `month`, DAYOFMONTH(post_date) AS `dayofmonth` FROM $wpdb->posts WHERE post_date < '$now' AND post_date != '0000-00-00 00:00:00' AND post_status = 'publish'"
            . ((wp_xoops_ismu()
                && !empty($GLOBALS['wp_xoops_author'])) ? ' AND post_author = '
                                                          . (int)$GLOBALS['wp_xoops_author'] : '')
        ...
        $arcresults = $wpdb->get_results(
            "SELECT DISTINCT WEEK(post_date, $start_of_week) AS `week`, YEAR(post_date) AS yr, DATE_FORMAT(post_date, '%Y-%m-%d') AS yyyymmdd FROM $wpdb->posts WHERE post_date < '$now' AND post_status = 'publish'"
            . ((wp_xoops_ismu() && !empty($GLOBALS['wp_xoops_author'])) ? ' AND post_author = '
                                                                          . (int)$GLOBALS['wp_xoops_author'] : '')
            . ' ORDER BY post_date DESC'
            . $limit
        ...
        $arcresults = $wpdb->get_results("SELECT * FROM $wpdb->posts WHERE post_date < '$now' AND post_status = 'publish'" . ((wp_xoops_ismu() && !empty($GLOBALS['wp_xoops_author'])) ? ' AND post_author = ' . (int)$GLOBALS['wp_xoops_author'] : '') . ' ORDER BY post_date DESC' . $limit);
...
function get_calendar($daylength = 1)
{
    ...
    $week_begins = (int)get_settings('start_of_week');
    ...
    if (!empty($monthnum) && !empty($year)) {
        ...
        $thisyear = '' . (int)mb_substr($m, 0, 4);

        $d = (($w - 1) * 7) + 6; //it seems MySQL's weeks disagree with PHP's

        $thismonth = $wpdb->get_var("SELECT DATE_FORMAT((DATE_ADD('${thisyear}0101', INTERVAL $d DAY) ), '%m')");
    } elseif (!empty($m)) {
        $calendar = mb_substr($m, 0, 6);
        ...
        if (mb_strlen($m) < 6) {
            ...
            $thismonth = '' . zeroise((int)mb_substr($m, 4, 2), 2);
        }
    } else {
    ...
    if ($previous) {
        echo "\n\t\t" . '<td abbr="' . $month[zeroise($previous->month, 2)] . '" colspan="3" id="prev"><a href="' . get_month_link($previous->year, $previous->month) . '" title="' . sprintf(
            __('View posts for %1$s %2$s'),
            $month[zeroise($previous->month, 2)],
            date('Y', mktime(0, 0, 0, $previous->month, 1, $previous->year))
        ) . '">&laquo; ' . $month_abbrev[$month[zeroise($previous->month, 2)]] . '</a></td>';
    } else {
    ...
    if ($next) {
        echo "\n\t\t" . '<td abbr="' . $month[zeroise($next->month, 2)] . '" colspan="3" id="next"><a href="' . get_month_link($next->year, $next->month) . '" title="' . sprintf(
            __('View posts for %1$s %2$s'),
    ...
    if (mb_strstr($_SERVER['HTTP_USER_AGENT'], 'MSIE') || mb_strstr(mb_strtolower($_SERVER['HTTP_USER_AGENT']), 'camino') || mb_strstr(mb_strtolower($_SERVER['HTTP_USER_AGENT']), 'safari')) {
        $ak_title_separator = "\n";
    } else {
        $ak_title_separator = ', ';
    }
    ...
    $ak_post_titles = $wpdb->get_results(
        'SELECT post_title, DAYOFMONTH(post_date) as dom ' . "FROM $wpdb->posts " . "WHERE YEAR(post_date) = '$thisyear' " . "AND MONTH(post_date) = '$thismonth' " . "AND post_date < '" . current_time('mysql') . "' " . "AND post_status = 'publish'"
    );
    ...
    if ($ak_post_titles) {
        foreach ($ak_post_titles as $ak_post_title) {
            ...
            if (empty($ak_titles_for_day[(string)$ak_post_title->dom])) { // first one
                $ak_titles_for_day[(string)$ak_post_title->dom] = str_replace('"', '&quot;', wptexturize($ak_post_title->post_title));
            } else {
                $ak_titles_for_day[(string)$ak_post_title->dom] .= $ak_title_separator . str_replace('"', '&quot;', wptexturize($ak_post_title->post_title));
            }
        }
    }
    ...
    for ($day = 1; $day <= $daysinmonth; ++$day) {
        ...
        if (in_array($day, $daywithpost, true)) { // any posts today?
...
function allowed_tags()
{
    ...
    return htmlentities($allowed, ENT_QUOTES | ENT_HTML5);
}
