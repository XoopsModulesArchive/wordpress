...
class WP_Query
{
    ...
    public function parse_query($query)
    {
        ...
        if (('' != $qv['attachment']) || (int)$qv['attachment_id']) {
            ...
            if ((int)$qv['m']) {
                ...
                if (mb_strlen($qv['m']) > 9) {
                    $this->is_time = true;
                } elseif (mb_strlen($qv['m']) > 7) {
                    $this->is_day = true;
                } elseif (mb_strlen($qv['m']) > 5) {
                    $this->is_month = true;
                } else {
                    $this->is_year = true;
                }
            ...
            if (empty($qv['cat']) || ('0' == $qv['cat'])) {
                $this->is_category = false;
            } else {
                if (mb_stristr($qv['cat'], '-')) {
                    $this->is_category = false;
                } else {
        ...
        if (mb_strstr($_SERVER['PHP_SELF'], 'wp-admin/')) {
            $this->is_admin = true;
        }
    ...
    public function &get_posts()
    {
        ...
        if (!isset($q['posts_per_page']) || 0 == $q['posts_per_page']) {
        ...
        $add_hours = (int)get_settings('gmt_offset');

        $add_minutes = (int)(60 * (get_settings('gmt_offset') - $add_hours));
        ...
        if ((int)$q['m']) {
            ...
            $where .= ' AND YEAR(post_date)=' . mb_substr($q['m'], 0, 4);

            if (mb_strlen($q['m']) > 5) {
                $where .= ' AND MONTH(post_date)=' . mb_substr($q['m'], 4, 2);
            }

            if (mb_strlen($q['m']) > 7) {
                $where .= ' AND DAYOFMONTH(post_date)=' . mb_substr($q['m'], 6, 2);
            }

            if (mb_strlen($q['m']) > 9) {
                $where .= ' AND HOUR(post_date)=' . mb_substr($q['m'], 8, 2);
            }

            if (mb_strlen($q['m']) > 11) {
                $where .= ' AND MINUTE(post_date)=' . mb_substr($q['m'], 10, 2);
            }

            if (mb_strlen($q['m']) > 13) {
                $where .= ' AND SECOND(post_date)=' . mb_substr($q['m'], 12, 2);
            }
        }
        ...
        if ((int)$q['minute']) {
            $q['minute'] = '' . (int)$q['minute'];

            $where .= " AND MINUTE(post_date)='" . $q['minute'] . "'";
        }

        if ((int)$q['second']) {
            $q['second'] = '' . (int)$q['second'];
        ...
        if ((int)$q['comments_popup']) {
            $q['p'] = (int)$q['comments_popup'];
        }

        // If a attachment is requested by number, let it supercede any post number.

        if (('' != $q['attachment_id']) && (0 != (int)$q['attachment_id'])) {
        ...
        if (('' != $q['page_id']) && (0 != (int)$q['page_id'])) {
            $q['page_id'] = (int)$q['page_id'];
        ...
        if (!empty($q['s'])) {
            ...
            if (!$q['sentence']) {
                ...
                for ($i = 1, $iMax = count($s_array); $i < $iMax; $i += 1) {
        ...
        if ((empty($q['cat'])) || ('0' == $q['cat'])
            ...
            $cat_array = preg_preg_split('/[,\s]+/', $q['cat']);

            $whichcat .= ' AND (category_id ' . $eq . ' ' . (int)$cat_array[0];

            $whichcat .= get_category_children($cat_array[0], ' ' . $andor . ' category_id ' . $eq . ' ');

            for ($i = 1; $i < (count($cat_array)); $i += 1) {
                $whichcat .= ' ' . $andor . ' category_id ' . $eq . ' ' . (int)$cat_array[$i];
        ...
        if ((empty($q['author'])) || ('0' == $q['author'])) {
            ...
            $author_array = preg_preg_split('/[,\s]+/', $q['author']);

            $whichauthor .= ' AND (post_author ' . $eq . ' ' . (int)$author_array[0];

            for ($i = 1; $i < (count($author_array)); $i += 1) {
        ...
        if ('' != $q['author_name']) {
            ...
            $whichauthor .= ' AND (post_author = ' . (int)$q['author'] . ')';
        }

        $where .= $search . $whichcat . $whichauthor;

        if ((empty($q['order'])) || (('ASC' != mb_strtoupper($q['order'])) && ('DESC' != mb_strtoupper($q['order'])))) {
            $q['order'] = 'DESC';
        }
        ...
        if (empty($q['orderby'])) {
            ...
            $allowed_keys = ['author', 'date', 'category', 'title', 'modified'];
            ...
            if (count($orderby_array) > 1) {
                for ($i = 1; $i < (count($orderby_array)); $i += 1) {
        ...
        if ('post.php' != $pagenow && 'edit.php' != $pagenow && !($this->is_single && $user_ID)) {
        ...
        if (empty($q['nopaging']) && !$this->is_single && !$this->is_page) {
            ...
            if (('posts' == $q['what_to_show'])) {
                $q['offset'] = abs((int)$q['offset']);

                if (empty($q['offset'])) {
                    $pgstrt = '';

                    $pgstrt = ((int)$page - 1) * $q['posts_per_page'] . ', ';
            ...
            } elseif ('days' == $q['what_to_show']) {
                $startrow = $q['posts_per_page'] * ((int)$page - 1);
...
class retrospam_mgr
{
    ...
    public function move_spam($id_list)
    {
        global $wpdb;
    ...
    public function find_spam()
    {
        $in_queue = 0;

        foreach ($this->comment_list as $comment) {
            if (1 == $comment->approved) {
                foreach ($this->spam_words as $word) {
                    ...
                    $fulltext = mb_strtolower($comment->email . ' ' . $comment->url . ' ' . $comment->ip . ' ' . $comment->text);

                    if (false !== mb_strpos($fulltext, mb_strtolower($word))) {
        ...
        return ['found' => $this->found_comments, 'in_queue' => $in_queue];
    }

    // End function find_spam

    public function display_edit_form($counters)
    {
...
class WP_Rewrite
{
    ...
    public function get_date_permastruct()
    {
        ...
        foreach ($endians as $endian) {
            if (false !== mb_strpos($this->permalink_structure, $endian)) {
                $date_endian = $endian;
    ...
    public function generate_rewrite_rules($permalink_structure, $paged = true, $feed = true, $forcomments = false, $walk_dirs = true)
    {
        $feedregex2 = '';
        ...
        $front = mb_substr($permalink_structure, 0, mb_strpos($permalink_structure, '%'));
        ...
        for ($j = 0; $j < $num_dirs; ++$j) {
            ...
            if ($feed) {
                $rewrite = [$feedmatch => $feedquery, $feedmatch2 => $feedquery2];
            }
            ...
            if ($num_toks) {
                ...
                if (mb_strstr($struct, '%postname%') || mb_strstr($struct, '%post_id%')
                    || mb_strstr($struct, '%pagename%')
                    || (mb_strstr($struct, '%year%') && mb_strstr($struct, '%monthnum%') && mb_strstr($struct, '%day%') && mb_strstr($struct, '%hour%') && mb_strstr($struct, '%minute') && mb_strstr($struct, '%second%'))) {
                    $post = true;

                    if (mb_strstr($struct, '%pagename%')) {
                        $page = true;
                    }
                ...
                if ($post) {
                    ...
                    if (!$page) {
                        $rewrite = array_merge($rewrite, [$sub1 => $subquery, $sub1tb => $subtbquery, $sub1feed => $subfeedquery, $sub1feed2 => $subfeedquery]);
                    }

                    $rewrite = array_merge($rewrite, [$sub2 => $subquery, $sub2tb => $subtbquery, $sub2feed => $subfeedquery, $sub2feed2 => $subfeedquery]);
                }
            }
    ...
    public function init()
    {
        ...
        $this->front = mb_substr($this->permalink_structure, 0, mb_strpos($this->permalink_structure, '%'));
...
class WP
{
    ...
    public $private_query_vars = ['posts_per_page', 'posts_per_archive_page', 'what_to_show', 'showposts', 'nopaging', 'show_post_type'];
    ...
    public function parse_request($extra_query_vars = '')
    {
        ...
        if (!empty($rewrite)) {
            ...
            foreach ($rewrite as $match => $query) {
                ...
                if ((!empty($req_uri)) && (0 === mb_strpos($match, $req_uri)) && ($req_uri != $request)) {
            ...
            if (empty($request) || $req_uri == $self || mb_strstr($_SERVER['PHP_SELF'], 'wp-admin/')) {
                if (isset($_GET['error'])) {
                    unset($_GET['error']);
                }
                ...
                if (isset($query_vars) && mb_strstr($_SERVER['PHP_SELF'], 'wp-admin/')) {
                    unset($query_vars);
                }
        ...
        for ($i = 0, $iMax = count($this->public_query_vars); $i < $iMax; $i += 1) {
    ...
    public function build_query_string()
    {
        ...
        foreach ($this->public_query_vars as $wpvar) {
            if (isset($this->query_vars[$wpvar]) && '' != $this->query_vars[$wpvar]) {
                $this->query_string .= (mb_strlen($this->query_string) < 1) ? '' : '&';
    ...
    public function handle_404()
    {
        ...
        if ((0 == count($wp_query->posts)) && !is_404() && !is_search() && ($this->did_permalink || (!empty($_SERVER['QUERY_STRING']) && (false === mb_strpos($_SERVER['REQUEST_URI'], '?'))))) {
            $wp_query->set_404();
