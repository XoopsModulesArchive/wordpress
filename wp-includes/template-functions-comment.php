...
function comments_number($zero = 'No Comments', $one = '1 Comment', $more = '% Comments', $number = '')
{
    ...
    if ('' == $number) {
        $number = $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->comments[$wp_id]} WHERE comment_post_ID = $id AND comment_approved = '1'");
    }
...
function comment_author_url_link($linktext = '', $before = '', $after = '')
{
    ...
    if ((!empty($url)) && ('http://' != $url) && ('http://url' != $url)) {
...
function comment_author_rss()
{
    ...
    if (empty($comment->comment_author)) {
        ...
        echo wp_convert_rss_charset(htmlspecialchars(apply_filters('comment_author', $comment->comment_author), ENT_QUOTES | ENT_HTML5));
    }
}
...
function comment_text_rss()
{
    ...
    $comment_text = htmlspecialchars($comment_text, ENT_QUOTES | ENT_HTML5);
