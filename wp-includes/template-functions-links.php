...
function get_permalink($id = 0)
{
    ...
    } elseif ('attachment' == $post->post_status) {
    ...
    if ('' != $permalink && 'draft' != $post->post_status) {
        ...
        if (mb_strstr($permalink, '%category%')) {
...
function get_feed_link($feed = 'rss2')
{
    ...
    if ('' != $permalink) {
        if (false !== mb_strpos($feed, 'comments_')) {
...
function get_previous_post($in_same_cat = false, $excluded_categories = '')
{
    ...
    if ($in_same_cat) {
        ...
        $join .= ' AND (category_id = ' . (int)$cat_array[0]->cat_ID;

        for ($i = 1; $i < (count($cat_array)); $i++) {
            $join .= ' OR category_id = ' . (int)$cat_array[$i]->cat_ID;
        }
    ...
    return @$wpdb->get_row(
        "SELECT ID, post_title FROM $wpdb->posts $join WHERE post_date < '$current_post_date' AND post_status = 'publish' " . (empty($GLOBALS['wp_xoops_author']) ? '' : ' AND post_author = ' . (int)$GLOBALS['wp_xoops_author']) . " $sqlcat $sql_exclude_cats ORDER BY post_date DESC LIMIT 1"
    );
}
...
function get_next_post($in_same_cat = false, $excluded_categories = '')
{
    ...
    return @$wpdb->get_row(
        "SELECT ID,post_title FROM $wpdb->posts $join WHERE post_date > '$current_post_date' AND post_date < '$now' AND post_status = 'publish' "
        . (empty($GLOBALS['wp_xoops_author']) ? '' : ' AND post_author = ' . (int)$GLOBALS['wp_xoops_author'])
...
function get_pagenum_link($pagenum = 1)
{
    ...
    if (mb_stristr($qstr, $page_querystring)) {
