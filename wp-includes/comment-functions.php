...
function clean_url($url)
{
    ...
    $url = (!mb_strstr($url, '://')) ? 'http://' . $url : $url;
...
function trackback_rdf($timezone = 0)
{
    ...
    if (!mb_stristr($_SERVER['HTTP_USER_AGENT'], 'W3C_Validator')) {
...
{ // less flexible, but saves DB queries
...
function pingback($content, $post_ID)
{
    ...
    foreach ($post_links_temp[0] as $link_test) :
        if (!in_array($link_test, $pung, true) && (url_to_postid($link_test) != $post_ID) // If we haven't pung it already and it isn't a link to itself
...
function discover_pingback_server_uri($url, $timeout_bytes = 2048)
{
    ...
    while (!feof($fp)) {
        ...
        $x_pingback_header_offset = mb_strpos(mb_strtolower($headers), $x_pingback_str);
        ...
        if (mb_strpos(mb_strtolower($headers), 'content-type: ')) {
    ...
    while (!feof($fp)) {
        ...
        $pingback_link_offset_dquote = mb_strpos($contents, $pingback_str_dquote);

        $pingback_link_offset_squote = mb_strpos($contents, $pingback_str_squote);
        ...
        if ($pingback_link_offset_dquote || $pingback_link_offset_squote) {
            ...
            $pingback_href_pos = @mb_strpos($contents, 'href=', $pingback_link_offset);

            $pingback_href_start = $pingback_href_pos + 6;

            $pingback_href_end = @mb_strpos($contents, $quote, $pingback_href_start);

            $pingback_server_url_len = $pingback_href_end - $pingback_href_start;

            $pingback_server_url = mb_substr($contents, $pingback_href_start, $pingback_server_url_len);
...
{
    if (!mb_strstr($url, get_bloginfo('home'))) {
        return false;
    }

    if (mb_strstr($url, get_bloginfo('home') . '/?attachment_id=')) {
        return true;
    }
...
function check_comment($author, $email, $url, $comment, $user_ip, $user_agent, $comment_type)
{
    ...
    if (1 == get_settings('comment_whitelist')) {
        if ('trackback' == $comment_type || 'pingback' == $comment_type) { // check if domain is in blogroll
            ...
            if ((1 == $ok_to_comment)
                && (empty($mod_keys) || false === mb_strpos($email, $mod_keys))) {
                return true;
            }
