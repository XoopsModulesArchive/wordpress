...
function get_pages($args = '')
{
    ...
    if (!empty($r['exclude'])) {
        $expages = preg_preg_split('/[\s,]+/', $r['exclude']);

        if (count($expages)) {
            foreach ($expages as $expage) {
                $exclusions .= ' AND ID <> ' . (int)$expage . ' ';
            }
        }
    }

    $pages = $wpdb->get_results(
        'SELECT * ' . "FROM $wpdb->posts " . "WHERE post_status = 'static' " . ((wp_xoops_ismu() && !empty($GLOBALS['wp_xoops_author'])) ? ' AND post_author = ' . (int)$GLOBALS['wp_xoops_author'] : '') . "$exclusions " . 'ORDER BY ' . $r['sort_column'] . ' ' . $r['sort_order']
    );
...
function _page_level_out($parent, $page_tree, $args, $depth = 0, $echo = true)
{
    ...
    foreach ($page_tree[$parent]['children'] as $page_id) {
        ...
        if (isset($cur_page['ts'])) {
            $format = $args['date_format'] ?? get_settings('date_format');
...
function get_attachment_icon($id = 0, $fullsize = false, $max_dims = false)
{
    ...
    if (!$fullsize && !empty($imagedata['thumb'])
    ...
    } elseif ('image/' == mb_substr($mime, 0, 6)
              && file_exists($file)) {
        ...
        $types = [mb_substr($mime, 0, mb_strpos($mime, '/')), mb_substr($mime, mb_strpos($mime, '/') + 1), str_replace('/', '_', $mime)];
...
function prepend_attachment($content)
{
    ...
    $p .= get_the_attachment_link(false, true, [400, 300]);
