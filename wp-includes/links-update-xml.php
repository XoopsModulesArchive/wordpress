...
function preload_links()
{
    ...
    foreach ($links as $link) {
        ...
        $all_links[$link_url] = [$link->link_id, 0];
    }
}
...
function get_weblogs_updatedfile()
{
    ...
    if ($update) {
        ...
        if (false !== $a && count($a) && $a[0]) {
            ...
            $cachefp = fopen(XOOPS_ROOT_PATH . '/modules/wordpress' . (('-' == $wp_id) ? '' : $wp_id) . '/' . get_settings('weblogs_cache_file'), 'wb');
...
if (get_weblogs_updatedfile()) {
    ...
    $fp = fopen(XOOPS_ROOT_PATH . '/modules/wordpress' . (('-' == $wp_id) ? '' : $wp_id) . '/' . get_settings('weblogs_cache_file'), 'rb') || die('Error reading XML data.');
    ...
    while ($data = fread($fp, 16384)) {
        ...
        xml_parse($xml_parser, $data, feof($fp)) || die(
        sprintf(
