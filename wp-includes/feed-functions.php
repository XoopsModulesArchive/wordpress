...
function get_author_rss_link($echo, $author_id, $author_nicename)
{
    $auth_ID = $author_id;
...
function get_category_rss_link($echo, $cat_ID, $category_nicename)
{
    $permalink_structure = get_settings('permalink_structure');
            ...
            if ('enclosure' == $key) {
                if (is_array($val)) {
                    foreach ($val as $enc) {
                        ...
                        print "<enclosure url='" . trim(htmlspecialchars($enclosure[0], ENT_QUOTES | ENT_HTML5)) . "' length='" . trim($enclosure[1]) . "' type='" . trim($enclosure[2]) . "'>\n";
                    }
                }
            }
        }
