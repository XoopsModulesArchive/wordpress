...
switch ($action) {
    case 'logout':
        ...
        $redirect_to = $_REQUEST['redirect_to'] ?? 'wp-login.php';

        //wp_redirect($redirect_to);
        wp_redirect(XOOPS_URL . '/user.php?op=logout');
        exit();
    ...
    case 'retrievepassword':
        ...
        $key = mb_substr(md5(uniqid(microtime())), 0, 50);
    ...
    case 'resetpass':
        ...
        // Generate something random for a password... md5'ing current time with a rand salt
        $key = str_ireplace("a-z0-9", '', $_GET['key']);
        if (empty($key)) {
        ...
        $new_pass = mb_substr(md5(uniqid(microtime())), 0, 7);
    ...
    default:
        ...
        if (is_object($GLOBALS['xoopsUser'])) {
            $_POST['log'] = $GLOBALS['xoopsUser']->getVar('uname');

            $_POST['pwd'] = $GLOBALS['xoopsUser']->getVar('pass');
        } else {
            wp_redirect(XOOPS_URL . '/user.php?xoops_redirect=' . $_SERVER['REQUEST_URI']);
        }
    ...
    $redirect_to = $_REQUEST['redirect_to'] ?? 'wp-admin/';
        ...
        if ($user_login && $user_pass) {
            ...
            if (!$user->has_cap('edit_posts') && (empty($redirect_to) || 'wp-admin/' == $redirect_to)) {
