...
if (get_xoops_option('wordpress' . (('-' == $wp_id) ? '' : $wp_id), 'wp_use_xoops_smilies')) {
    ...
    if ('0' == ($numsmiles = $db->getRowsNum($getsmiles))) {
        //EMPTY
    } else {
...
if (file_exists(__DIR__ . '/themes/' . $xoopsConfig['theme_set'] . '/wp-config-custom.php')) {
    $themes = $xoopsConfig['theme_set'];
} else {
    $themes = 'default';
}
include __DIR__ . '/themes/' . $themes . '/wp-config-custom.php';
