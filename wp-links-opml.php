...
if ((empty($link_cat)) || ('all' == $link_cat) || ('0' == $link_cat)) {
    $sql_cat = '';
} else { // be safe
    ...
    if ($results) {
        foreach ($results as $result) {
            if ($result->link_category != $prev_cat_id) { // new category
                if (0 != $prev_cat_id) { // not first time
                    ?>
                    </outline>
                    <?php
            ...
            <outline text="<?php echo wp_specialchars($result->link_name); ?>" type="link" xmlUrl="<?php echo wp_specialchars($result->link_rss); ?>" htmlUrl="<?php echo wp_specialchars($result->link_url); ?>" updated="<?php if ('0000-00-00 00:00:00' != $result->link_updated) {
                echo $result->link_updated;
            } ?>">
            <?php
        } // end foreach
        ?>
