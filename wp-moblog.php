...
if (!($mbox = imap_open((string)$mailserver, (string)$mailserver_login, (string)$mailserver_pass))) {
    die('error opening mailbox');
}

#Check to see if there is any mail in there
if (imap_num_msg($mbox) < 1) {
    die();
} //no mail found, no need for error log
...
for ($iCount = 1; $iCount <= $Count; $iCount++) {
    ...
    $ddate_U += ($mail_time_difference * 3600);
    ...
    if (preg_match('/' . $subjectprefix . '/', $subject)) {
        ...
        $btpos = mb_strpos($content, $bodyterminator);
        ...
        if ($use_phoneemail) {
            $btpos = mb_strpos($firstline, $phoneemail_separator);

            if ($btpos) {
                $userpassstring = trim(mb_substr($firstline, 0, $btpos));

                $content = trim(mb_substr($content, $btpos + mb_strlen($phoneemail_separator), mb_strlen($content)));

                $btpos = mb_strpos($content, $phoneemail_separator);
...
function parse($structure)
{
    ...
    for ($x = 0, $xMax = count($parts); $x < $xMax; $x++) {
        ...
        $ret[$x]['type'] = $type[$this->type] . '/' . mb_strtolower($this->subtype);
        ...
        $ret[$x]['size'] = mb_strtolower($this->bytes);

        $ret[$x]['disposition'] = mb_strtolower($this->disposition);

        if ('attachment' == mb_strtolower($this->disposition)) {
            $params = $this->dparameters;

            foreach ($params as $p) {
                if ('FILENAME' == $p->attribute) {
                    $ret[$x]['name'] = $p->value;
...
function get_attachments($arr)
{
    for ($x = 0, $xMax = count($arr); $x < $xMax; $x++) {
        if ('attachment' == $arr[$x]['disposition']) {
            $ret[] = $arr[$x];
        }
    }
