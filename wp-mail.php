...
for ($i = 1; $i <= $count; $i++) :
    ...
    foreach ($message as $line) :
        ...
        if ($bodysignal) {
            ...
            if (preg_match('/Content-Type: /i', $line)) {
                ...
                $content_type = mb_substr($content_type, 14, mb_strlen($content_type) - 14);
            ...
            if (('multipart/alternative' == $content_type) && (preg_match('/boundary="/', $line)) && ('' == $boundary)) {
                $boundary = trim($line);
            ...
            if (preg_match('/From: /', $line) | preg_match('Reply-To: /', $line)) {
                ...
                if (preg_match("([a-zA-Z0-9\_\-\.]+@[\a-zA-z0-9\_\-\.]+)", $author, $regs)) {
                    $author = $regs[1];
    ...
    if (empty($post_categories)) {
        $post_categories[] = get_settings('default_email_category');
    }
