...
$rand = mb_substr(md5(md5(DB_PASSWORD)), -5);
...
class wpdbBackup
{
    public $backup_complete = false;

    public $backup_file = '';

    public $backup_dir = WP_BACKUP_DIR;
    ...
    {
        add_action('wp_cron_daily', [&$this, 'wp_cron_daily']);
        ...
        if (isset($_POST['do_backup'])) {
            if (!current_user_can('import')) {
                die(__('You are not allowed to perform backups.'));
            }
            ...
            switch ($_POST['do_backup']) {
                case 'backup':
                    ...
                    break;
                case 'fragments':
                    add_action('admin_menu', [&$this, 'fragment_menu']);
                    break;
            }
    ...
    public function init()
    {
        ...
        if (isset($_GET['fragment'])) {
            [$table, $segment, $filename] = explode(':', $_GET['fragment']);
    ...
    public function build_backup_script()
    {
        ...
        echo '<h2>' . __('Backup') . '</h2>
			...
			<p><strong>' . __('DO NOT DO THE FOLLOWING AS IT WILL CAUSE YOUR BACKUP TO FAIL:') . '</strong></p>
			<ol>
    ...
    public function backup_fragment($table, $segment, $filename)
    {
        global $table_prefix, $wpdb;
    ...
    {
        add_management_page(__('Backup'), __('Backup'), 'import', basename(__FILE__), [&$this, 'backup_menu']);
    }

    public function fragment_menu()
    {
        add_management_page(__('Backup'), __('Backup'), 'import', basename(__FILE__), [&$this, 'build_backup_script']);
    }
    ...
    public function backup_error($err)
    {
        if (count($this->backup_errors) < 20) {
            $this->backup_errors[] = $err;
        } elseif (20 == count($this->backup_errors)) {
        ...
        */
        ...
        if (('none' == $segment) || ($segment >= 0)) {
            ...
            foreach ($table_structure as $struct) {
                if ((0 === mb_strpos($struct->Type, 'tinyint'))
                    || (0 === mb_strpos(mb_strtolower($struct->Type), 'smallint'))
                    || (0 === mb_strpos(mb_strtolower($struct->Type), 'mediumint'))
                    || (0 === mb_strpos(mb_strtolower($struct->Type), 'int'))
                    || (0 === mb_strpos(mb_strtolower($struct->Type), 'bigint'))
                    || (0 === mb_strpos(mb_strtolower($struct->Type), 'timestamp'))) {
                    $ints[mb_strtolower($struct->Field)] = '1';
                }
            }
            ...
            } while ((count($table_data) > 0) and ('none' == $segment));
        }
    ...
    public function db_backup($core_tables, $other_tables)
    {
        global $table_prefix, $wpdb;
    ...
    public function deliver_backup($filename = '', $delivery = 'http', $recipient = '')
    {
        if ('' == $filename) {
            return false;
        }
        ...
        if ('http' == $delivery) {
            ...
            $data = chunk_preg_split(base64_encode($file));
    ...
    public function backup_menu()
    {
        ...
        if ((function_exists('wp_cron_init')) && isset($_POST['wp_cron_backup_options'])) {
            update_option('wp_cron_backup_schedule', (int)$_POST['cron_schedule'], false);
        ...
        echo '<label style="display:block;"><input type="radio" checked name="deliver" value="http"> ' . __('Download to your computer') . '</label>';
        ...
        foreach ($table_status as $table) {
            ...
            if (in_array($table->Name, $wp_backup_default_tables, true)) {
        ...
        $mem_limit = (0 == $mem_limit) ? 8 * 1024 * 1024 : $mem_limit - 2000000;
        ...
        if (function_exists('wp_cron_init')) {
            ...
            if (count($other_tables) > 0) {
                ...
                foreach ($other_tables as $table) {
                    ...
                    if (in_array($table, $cron_tables, true)) {
    ...
    public function wp_cron_daily()
    {
        $schedule = (int)get_option('wp_cron_backup_schedule');
