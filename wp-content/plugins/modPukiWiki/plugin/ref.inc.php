<?php
// $Id: ref.inc.php,v 1.1 2006/03/10 18:50:37 mikhail Exp $
/*
...
if (!defined('MOD_PUKI_REF_GD_VERSION')) {
    define('MOD_PUKI_REF_GD_VERSION', 2);
} // Ver 2

// file icon image
if (!defined('MOD_PUKI_REF_FILE_ICON')) {
    define('MOD_PUKI_REF_FILE_ICON', '<img src="' . MOD_PUKI_FILE_ICON . '" alt="file" width="20" height="20">');
}

// default alignment
if (!defined('MOD_PUKI_REF_DEFAULT_ALIGN')) {
    define('MOD_PUKI_REF_DEFAULT_ALIGN', 'left');
} // 'left','center','right'

// force wrap on default
if (!defined('MOD_PUKI_REF_WRAP_TABLE')) {
    define('MOD_PUKI_REF_WRAP_TABLE', false);
} // TRUE,FALSE
...
function plugin_ref_inline()
{
    ...
    //パラメータ変換
    $params = ['_args' => [], 'nocache' => false, '_size' => false, '_w' => 0, '_h' => 0, '_%' => ''];
...
function plugin_ref_convert()
{
    ...
    //パラメータ変換
    $params = ['left' => false, 'center' => false, 'right' => false, 'wrap' => false, 'nowrap' => false, 'around' => false, '_args' => [], '_done' => false, 'nocache' => false, '_size' => false, '_w' => 0, '_h' => 0, '_%' => ''];
...
function plugin_ref_body($name, $args, $params)
{
    ...
    if (PukiWikiFunc::is_url($name)) { //URL
        $url   = $ext = $info = htmlspecialchars($name, ENT_QUOTES | ENT_HTML5);
        $icon  = $size = '';
        $l_url = $url;
    ...
    if ($is_picture) { // 画像
        ...
        foreach ($params['_args'] as $arg) {
            ...
            if (preg_match('/^t:(.*)$/i', $arg, $m)) {
                $m[1] = htmlspecialchars(str_replace('&amp;quot;', '', $m[1]), ENT_QUOTES | ENT_HTML5);
                if ($m[1]) {
                    $title = $m[1] . '&#13;&#10;' . $title;
                }
            }
