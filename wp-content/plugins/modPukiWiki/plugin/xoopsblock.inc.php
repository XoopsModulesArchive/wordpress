<?php
// $Id: xoopsblock.inc.php,v 1.1 2006/03/10 18:50:37 mikhail Exp $
...
function plugin_xoopsblock_convert()
{
    ...
    [$tgt, $option1, $option2] = func_get_args();
    ...
    if (preg_match("/^(around|float)(:?w?([\d]+%?))?$/i", $option2, $arg)) {
        ...
        $width = (!mb_strstr($arg[3], '%')) ? $arg[3] . 'px' : $arg[3];
    ...
    foreach ($arr as $myblock) {
        ...
        $block_type = ($myblock->getVar('type')) ?: $myblock->getVar('block_type');

        //$name = ($myblock->getVar("type") != "C") ? $myblock->getVar("name") : $myblock->getVar("title");

        $name = ('C' != $block_type) ? $myblock->getVar('name') : $myblock->getVar('title');
        ...
        if ('?' == $tgt) {
            ...
            if ($tgt_bid === $bid || $tgt == $name) {
                ...
                if (!is_object($xoopsTpl)) { // XOOPS 1 系用
                    $ret = $block['content'];
                } else {
                    // XOOPS 2 系用
