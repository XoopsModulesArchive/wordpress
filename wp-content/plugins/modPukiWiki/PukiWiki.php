...
if (file_exists('PukiWiki.ini.php')) {
    require __DIR__ . '/PukiWiki.ini.php';
} elseif (file_exists('PukiWiki.ini.dist.php')) {
    require __DIR__ . '/PukiWiki.ini.dist.php';
}

if (!defined('MOD_PUKI_SOURCE_ENCODING')) {
    define('MOD_PUKI_SOURCE_ENCODING', 'EUC-JP');
}
if (!defined('MOD_PUKI_ZONETIME')) {
    define('MOD_PUKI_ZONETIME', 9 * 3600);
} // JST = GMT+9
if (!defined('MOD_PUKI_ZONE')) {
    define('MOD_PUKI_ZONE', 'JST');
}

if (!defined('MOD_PUKI_LOCALZONE')) {
    define('MOD_PUKI_LOCALZONE', date('Z'));
}
if (!defined('MOD_PUKI_UTIME')) {
    define('MOD_PUKI_UTIME', time() - MOD_PUKI_LOCALZONE);
}

if (!defined('MOD_PUKI_BASE')) {
    define('MOD_PUKI_BASE', __DIR__);
...
if (defined('XOOPS_ROOT_PATH')) {
    ...
    if (file_exists(XOOPS_ROOT_PATH . '/modules/bwiki/xoops_version.php')) {
        ...
        if (!defined('MOD_PUKI_WIKI_URL')) {
            define('MOD_PUKI_WIKI_URL', XOOPS_URL . '/modules/bwiki/index.php');
        }

        if (!defined('MOD_PUKI_WIKI_DATA_DIR')) {
            define('MOD_PUKI_WIKI_DATA_DIR', XOOPS_ROOT_PATH . '/modules/bwiki/wiki/');
        }

        if (!defined('MOD_PUKI_WIKI_CACHE_DIR')) {
            define('MOD_PUKI_WIKI_CACHE_DIR', XOOPS_ROOT_PATH . '/modules/bwiki/cache/');
        }

        if (!defined('MOD_PUKI_WIKI_UPLOAD_DIR')) {
            define('MOD_PUKI_WIKI_UPLOAD_DIR', XOOPS_ROOT_PATH . '/modules/bwiki/attach/');
        }

        if (!defined('MOD_PUKI_WIKI_VER')) {
            define('MOD_PUKI_WIKI_VER', '1.4');
        }
    } elseif (file_exists(XOOPS_ROOT_PATH . '/modules/pukiwiki/xoops_version.php')) {
        //XOOPS環境下でPukiWikiModが導入されていた場合に自動検知してページリンク機能などを有効にするための定義を設定

        if (!defined('MOD_PUKI_WIKI_URL')) {
            define('MOD_PUKI_WIKI_URL', XOOPS_URL . '/modules/pukiwiki/index.php');
        }

        if (!defined('MOD_PUKI_WIKI_DATA_DIR')) {
            define('MOD_PUKI_WIKI_DATA_DIR', XOOPS_ROOT_PATH . '/modules/pukiwiki/wiki/');
        }

        if (!defined('MOD_PUKI_WIKI_CACHE_DIR')) {
            define('MOD_PUKI_WIKI_CACHE_DIR', XOOPS_ROOT_PATH . '/modules/pukiwiki/cache/');
        }

        if (!defined('MOD_PUKI_WIKI_UPLOAD_DIR')) {
            define('MOD_PUKI_WIKI_UPLOAD_DIR', XOOPS_ROOT_PATH . '/modules/pukiwiki/attach/');
        }

        if (!defined('MOD_PUKI_WIKI_VER')) {
            define('MOD_PUKI_WIKI_VER', '1.3');
        }
    }
    ...
    if (!defined('MOD_PUKI_UPLOAD_URL')) {
        define('MOD_PUKI_UPLOAD_URL', XOOPS_URL . '/uploads/modPukiWiki/');
    }
...
} elseif (defined('ABSPATH') and ('WPINC')) {
    ...
    if (!defined('MOD_PUKI_LANG')) {
        define('MOD_PUKI_LANG', 'ja');
    }
}
if (!defined('MOD_PUKI_LANG_DIR')) {
    define('MOD_PUKI_LANG_DIR', MOD_PUKI_LANG_BASE . '/' . MOD_PUKI_LANG);
}
...
if (defined('MOD_PUKI_UPLOAD_DIR')) {
    ...
    foreach ($modPukiImages as $modPukiIMG) {
        if (!file_exists(MOD_PUKI_UPLOAD_DIR . $modPukiIMG)) {
            copy(MOD_PUKI_IMAGE_DIR . $modPukiIMG, MOD_PUKI_UPLOAD_DIR . $modPukiIMG);
        }
    }

    if (!defined('MOD_PUKI_NOIMAGE')) {
        define('MOD_PUKI_NOIMAGE', MOD_PUKI_UPLOAD_URL . '/noimage.png');
    }

    if (!defined('MOD_PUKI_NOIMAGE_S')) {
        define('MOD_PUKI_NOIMAGE_S', MOD_PUKI_UPLOAD_URL . '/noimage_s.png');
    }

    if (!defined('MOD_PUKI_FILE_ICON')) {
        define('MOD_PUKI_FILE_ICON', MOD_PUKI_UPLOAD_URL . '/file.gif');
    }
}
