...
function ksd_auto_check_comment($comment)
{
    ...
    $comment['referrer'] = Request::getString('HTTP_REFERER', '', 'SERVER');
        ...
        if ($comments) {
            ...
            foreach ($comments as $comment) {
                ...
                echo "\n\t<li id='comment-$comment->comment_ID' $class>"; ?>
                        ...
                        <p><strong><?php comment_author() ?></strong> <?php if ($comment->comment_author_email) { ?>| <?php comment_author_email_link() ?><?php }

                if ($comment->comment_author_url && 'http://' != $comment->comment_author_url) { ?> | <?php comment_author_url_link() ?><?php } ?>| <?php _e('IP:') ?> <a href="http://ws.arin.net/cgi-bin/whois.pl?queryinput=<?php comment_author_IP() ?>"><?php comment_author_IP() ?></a>
                        </p>
        ...
        function widget_akismet($args)
        {
            ...
            <div id="akismetwrap">
                <div id="akismetstats"><a id="aka" href="http://akismet.com" title="">
                        <div id="akismet1"><span id="akismetcount"><?php echo $count; ?></span> <span id="akismetsc"><?php _e('spam comments') ?></span></div>
                        <div id="akismet2"><span id="akismetbb"><?php _e('blocked by') ?></span><br><span id="akismeta">Akismet</span></div>
                    </a></div>
            </div>
...
function akismet_recheck_queue()
{
    ...
    wp_redirect(Request::getString('HTTP_REFERER', '', 'SERVER'));
