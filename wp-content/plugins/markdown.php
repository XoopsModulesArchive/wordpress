...
$g_nested_brackets = str_repeat('(?>[^\[\]]+|\[', $g_nested_brackets_depth) . str_repeat('\])*', $g_nested_brackets_depth);
$g_escape_table = [
    '\\' => md5('\\'),
    '`' => md5('`'),
    '*' => md5('*'),
...
if (!defined('WP_PLUGIN_MARKDOWN')) {
    ...
    function _StripLinkDefinitions_callback($matches)
    {
        ...
        $link_id = mb_strtolower($matches[1]);

        $g_urls[$link_id] = _EncodeAmpsAndAngles($matches[2]);

        if (isset($matches[3])) {
            $g_titles[$link_id] = htmlentities($matches[3], ENT_QUOTES | ENT_HTML5);
        }
        ...
        foreach ($tokens as $cur_token) {
            if ('tag' == $cur_token[0]) {
                $cur_token[1] = str_replace(
                    ['*', '_'],
                    [$g_escape_table['*'], $g_escape_table['_']],
                    $cur_token[1]
                );
    ...
    function _DoAnchors_reference_callback($matches)
    {
        ...
        $link_id = mb_strtolower($matches[3]);

        if ('' == $link_id) {
            $link_id = mb_strtolower($link_text); # for shortcut links like [this][].
        }
    ...
    function _DoImages_reference_callback($matches)
    {
        ...
        if ('' == $link_id) {
            $link_id = mb_strtolower($alt_text); # for shortcut links like ![this][].
        }
    ...
    function _EncodeCode($_)
    {
        ...
        $_ = str_replace(
            array_keys($g_escape_table),
            array_values($g_escape_table),
            $_
        );
    ...
    function _EncodeBackslashEscapes($text)
    {
        ...
        return str_replace(
            array_keys($g_backslash_escape_table),
            array_values($g_backslash_escape_table),
            $text
        );
    }
    ...
    function _TokenizeHTML($str)
    {
        ...
        $match = "(?s:<!(--.*?--\s*)+>)|" .  # comment
                 ...
                 (string)$nested_tags;            # nested tags
        ...
        foreach ($matches as $element) {
            ...
            $sec_start = $tag_start + mb_strlen($whole_tag);

            if ($pos < $tag_start) {
                $tokens[] = [
                    'text',
                    mb_substr($str, $pos, $tag_start - $pos),
                ];
            }

            $tokens[] = ['tag', $whole_tag];
