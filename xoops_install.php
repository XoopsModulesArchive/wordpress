...
function xoops_module_install_WordPress($module)
{
    ...
    $my_wp_dirname = basename(__DIR__);

    if (!preg_match('/wordpress(\d*)/', $my_wp_dirname, $regs)) {
        echo('invalid dirname of wordpress: ' . htmlspecialchars($my_wp_dirname, ENT_QUOTES | ENT_HTML5));
    }
    ...
    $query = 'INSERT INTO '
             . $db->prefix("wp{$my_wp_dirnumber}_comments")
             . " (comment_post_ID, comment_author, comment_author_email, comment_author_url, comment_author_IP, comment_date, comment_content) VALUES ('1', 'NobuNobu Xoops', 'mail@example.com', 'http://www.kowa.org/', '127.0.0.1', '$now',"
             . _MI_WORDPRESS_INST_COMMENT_CONTENT
             . ')';

    $q = $db->query($query);

    $query = 'INSERT INTO '
             . $db->prefix("wp{$my_wp_dirnumber}_options")
             . " (`option_id`, `blog_id`, `option_name`, `option_can_override`, `option_type`, `option_value`, `option_width`, `option_height`, `option_description`, `option_admin_level`) VALUES (22, 0, 'rss_language', 'Y', 3, "
             . _MI_WORDPRESS_INST_OPTIONS_22
             . ", 20, 8, '_LANG_INST_BASE_VALUE22', 8)";

    $q = $db->query($query);

    $query = 'INSERT INTO '
             . $db->prefix("wp{$my_wp_dirnumber}_options")
             . " (`option_id`, `blog_id`, `option_name`, `option_can_override`, `option_type`, `option_value`, `option_width`, `option_height`, `option_description`, `option_admin_level`) VALUES (52, 0, 'date_format', 'Y', 3, "
             . _MI_WORDPRESS_INST_OPTIONS_52
             . ", 20, 8, '_LANG_INST_BASE_VALUE52', 4)";

    $q = $db->query($query);

    $query = 'INSERT INTO '
             . $db->prefix("wp{$my_wp_dirnumber}_options")
             . " (`option_id`, `blog_id`, `option_name`, `option_can_override`, `option_type`, `option_value`, `option_width`, `option_height`, `option_description`, `option_admin_level`) VALUES (53, 0, 'time_format', 'Y', 3, "
             . _MI_WORDPRESS_INST_OPTIONS_53
             . ", 20, 8, '_LANG_INST_BASE_VALUE53', 4)";

    $q = $db->query($query);

    $query = 'INSERT INTO '
             . $db->prefix("wp{$my_wp_dirnumber}_optionvalues")
             . " (`option_id`, `optionvalue`, `optionvalue_desc`, `optionvalue_max`, `optionvalue_min`, `optionvalue_seq`) VALUES (59, 'SELECT cat_id AS value, cat_name AS label FROM "
             . $db->prefix("wp{$my_wp_dirnumber}_categories")
             . " order by cat_name', '', NULL, NULL, 1)";
