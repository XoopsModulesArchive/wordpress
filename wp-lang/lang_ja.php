...
if (!defined('WP_LANGUAGE_FILE_READ')) {
    ...
    define(
        ...
        '何らかの理由でこのウィザードが機能しない場合でも心配要りません。データベース設定のすべては別ファイルにて行うことが出来ます。 <b>wp-config-sample.php</b> をテキストエディタで開き、必要情報を記述したうえで <b>wp-config.php</b> として保存後サーバーへアップロードしてください。</p><p>あなたのデータベース情報が不明な場合は、このウィザードを進める前にレンタルサーバー(ホスティング)サービスに問い合わせてください。すべて準備ができている場合は<a href="install-config.php?step=1">こちらをクリック</a>してください。'
    );
    ...
    define(
        '_LANG_EF_TRACK_FORM',
        '<p><label for="trackback"><a href="http://wordpress.xwd.jp/wiki/index.php?Reference%20Post%2FEdit#trackback" title="Help on trackbacks"><strong>TrackBack</strong> an <acronym title="Uniform Resource Locator">URL</acronym></a>:</label> (複数のURLがある場合は半角スペースで区切ってください)<br>'
    );
...
'
    ...
    define(
        '_LANG_WPF_SUBT_COPY',
        'One-click bookmarkletを有効にするには、<br>
...
詳しくは管理人迄お問い合わせください。</p>'
    ...
    define(
        ...
        '<p>サーバー上に <b>wp-config.php</b> ファイルが存在しません<br>WordPress ME のインストールにはこのファイルが必要です。<br><br>こちらの<a href="install-config.php">ウィザード</a>を利用してサーバー上で <b>wp-config.php</b> ファイルを作成することができますが、この方法はすべての環境での動作を保障することができませんのでご了承下さい。<br><br>最も確実な方法は <b>wp-config-sample.php</b> を参考に手動でファイルを作成することです。</p>'
    );
    ...
    define(
        ...
        '<br>ここでは、インストールの為の幾つかのステップをご案内します。<br>インストールを始める前に、少なくとも 4.0.6 のPHPバージョンが必要です。<br>
