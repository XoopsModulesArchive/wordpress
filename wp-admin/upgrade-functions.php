...
function upgrade_100()
{
    ...
    $wpdb->query(
        "UPDATE $wpdb->options SET option_value = REPLACE(option_value, 'wp-links/links-images/', 'wp-images/links/')
...
function upgrade_160()
{
    ...
    foreach ($users as $user) :
        ...
        if (empty($caps) || defined('RESET_CAPS')) {
            ...
            update_usermeta($user->ID, $table_prefix . 'capabilities', [$role => true]);
        }
    ...
    if ($wp_current_db_version > 2541 && $wp_current_db_version <= 3091) {
        ...
        foreach ($objects as $object) {
            $wpdb->query(
                "UPDATE $wpdb->posts SET post_status = 'attachment',
...
function make_site_theme_from_oldschool($theme_name, $template)
{
    ...
    foreach ($files as $oldfile => $newfile) {
        ...
        if ($oldfile == 'index.php') { // Check to make sure it's not a new index
            ...
            if (false !== strpos($index, 'WP_USE_THEMES')) {
...
function make_site_theme_from_default($theme_name, $template)
{
    ...
    $theme_dir = @ dir((string)$default_dir);
    if ($theme_dir) {
    ...
    if ($stylelines) {
        ...
        foreach ($stylelines as $line) {
            if (false !== strpos($line, 'Theme Name:')) {
                $line = "Theme Name: $theme_name";
            } elseif (false !== strpos($line, 'Theme URI:')) {
                $line = 'Theme URI: ' . __get_option('siteurl');
            } elseif (false !== strpos($line, 'Description:')) {
                $line = 'Description: Your theme';
            } elseif (false !== strpos($line, 'Version:')) {
                $line = 'Version: 1';
            } elseif (false !== strpos($line, 'Author:')) {
                $line = 'Author: You';
            }
            fwrite($f, "{$line}\n");
        }
