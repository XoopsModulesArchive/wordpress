...
switch ($action) {
    case 'assign':
    {
        ...
        $all_links = implode(',', $ids_to_change);
    ...
    case 'visibility':
    {
        ...
        foreach ($results as $row) {
            if ('Y' == $row->link_visible) { // ok to proceed
    ...
    if ('popup' != $action) {
        ...
        echo "        </select>\n"; ?>
                        ...
                        <td>
                            <select name="order_by">
                                <option value="order_id" <?php if ('order_id' == $order_by) {
            echo " selected='selected'";
        } ?>><?php _e('Link ID') ?></option>
                                <option value="order_name" <?php if ('order_name' == $order_by) {
            echo " selected='selected'";
        } ?>><?php _e('Name') ?></option>
                                <option value="order_url" <?php if ('order_url' == $order_by) {
            echo " selected='selected'";
        } ?>><?php _e('URI') ?></option>
                                <option value="order_desc" <?php if ('order_desc' == $order_by) {
            echo " selected='selected'";
        } ?>><?php _e('Description') ?></option>
                                <option value="order_owner" <?php if ('order_owner' == $order_by) {
            echo " selected='selected'";
        } ?>><?php _e('Owner') ?></option>
                                <option value="order_rating" <?php if ('order_rating' == $order_by) {
            echo " selected='selected'";
        } ?>><?php _e('Rating') ?></option>
                            </select>
                        </td>
                        <td>
        ...
        if ($links) {
            foreach ($links
                ...
                $image = (null != $link->link_image) ? __('Yes') : __('No');

                $visible = ('Y' == $link->link_visible) ? __('Yes') : __('No');
...
LINKS;
                ...
                if ($show_buttons) {
                    ...
                    echo '<td><a href="' . wp_nonce_url('link-manager.php?link_id=' . $link->link_id . '&amp;action=delete', 'delete-bookmark_' . $link->link_id) . '"' . " class='delete' onclick=\"return deleteSomething( 'link', $link->link_id , '" . sprintf(
                        __('You are about to delete the &quot;%s&quot; bookmark to %s.\\n&quot;Cancel&quot; to stop, &quot;OK&quot; to delete.'),
                        js_escape($link->link_name),
                        js_escape($link->link_url)
                    ) . '\' );">' . __('Delete') . '</a></td>';
            ...
            <div class="wrap">
                <table width="100%" cellpadding="3" cellspacing="3">
                    <tr>
                        <th colspan="4"><?php _e('Manage Multiple Links:') ?></th>
                    </tr>
                    <tr>
                        <td colspan="4"><?php _e('Use the checkboxes on the right to select multiple links and choose an action below:') ?></td>
                    </tr>
                    <tr>
                        <td>
        ...
        link_category_dropdown('category'); ?> <input name="move" type="submit" id="move" value="<?php _e('Go') ?>">
                        </td>
                        <td align="right">
