...
if (isset($_GET['action'])) {
    if ('activate' == $_GET['action']) {
        ...
        if (!in_array($_GET['plugin'], $current, true)) {
        ...
        array_splice($current, array_search($_GET['plugin'], $current, true), 1); // Array-fu!
...
foreach ($check_plugins as $check_plugin) {
    if (!file_exists(ABSPATH . 'wp-content/plugins/' . $check_plugin)) {
        ...
        $key = array_search($check_plugin, $current, true);
...
<div class="wrap">
    ...
    if (empty($plugins)) {
        ...
        foreach ($plugins as $plugin_file => $plugin_data) {
            ...
            if (!empty($current_plugins) && in_array($plugin_file, $current_plugins, true)) {
                ...
                $style .= 'alternate' == $style ? ' active' : 'active';
            } else {
            ...
            $plugin_data['Description'] = wp_kses($plugin_data['Description'], ['a' => ['href' => [], 'title' => []], 'abbr' => ['title' => []], 'acronym' => ['title' => []], 'code' => [], 'em' => [], 'strong' => []]);
    ...
    <p><?php _e(
        'You can find additional plugins for your site in the <a href="http://wordpress.org/extend/plugins/">WordPress plugin directory</a>. To install a plugin you generally just need to upload the plugin file into your <code>wp-content/plugins</code> directory. Once a plugin is uploaded, you may activate it here.'
    ); ?></p>
