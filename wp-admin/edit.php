...
<div class="wrap">
    <h2>
        ...
        if (is_month()) {
            ...
            } elseif (!is_paged() || 1 == get_query_var('paged')) {
                _e('Last 15 Posts');
            } else {
                _e('Previous Posts');
            }
    ...
    <form name="searchform" action="" method="get" style="float: left; width: 16em; margin-right: 3em;">
        <fieldset>
            ...
            <input type="text" name="s" value="<?php if (isset($s)) {
                    ...
                    foreach ($posts_columns as $column_name => $column_display_name) {
                        switch ($column_name) {
                            ...
                            case 'control_delete':
                                ?>
                                <td><?php if (current_user_can('edit_post', $post->ID)) {
                                    echo "<a href='" . wp_nonce_url("post.php?action=delete&amp;post=$id", 'delete-post_' . $post->ID) . "' class='delete' onclick=\"return deleteSomething( 'post', " . $id . ", '" . sprintf(
    ...
    <ol id="comments">
        ...
        foreach ($comments as $comment) {
            ...
            <li <?php if ('unapproved' == $comment_status) {
                ...
                if (current_user_can('edit_post', $post->ID)) {
                    ...
                    echo ' - <a href="' . wp_nonce_url('post.php?action=deletecomment&amp;p=' . $post->ID . '&amp;comment=' . $comment->comment_ID, 'delete-comment_' . $comment->comment_ID) . '" onclick="return confirm(\'' . __(
