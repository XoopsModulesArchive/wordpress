...
if (empty($_GET['mode'])) {
    ...
    $mode = wp_specialchars($_GET['mode'], 1);
}
?>
...
<div class="wrap">
    ...
    if (isset($_GET['s'])) {
        ...
        $comments = $wpdb->get_results(
            "SELECT * FROM $wpdb->comments  WHERE
    ...
    if ('view' == $mode) {
        if ($comments) {
            ...
            foreach ($comments as $comment) {
                ...
                <p><strong><?php _e('Name:') ?></strong> <?php comment_author() ?> <?php if ($comment->comment_author_email) { ?>| <strong><?php _e('E-mail:') ?></strong> <?php comment_author_email_link() ?><?php }
                ...
                if ($comment->comment_author_url && 'http://' != $comment->comment_author_url) { ?> | <strong><?php _e('URI:') ?></strong> <?php comment_author_url_link() ?><?php } ?>| <strong><?php _e('IP:') ?></strong> <a
                            href="http://ws.arin.net/cgi-bin/whois.pl?queryinput=<?php comment_author_IP() ?>"><?php comment_author_IP() ?></a></p>
                ...
                if (current_user_can('edit_post', $comment->comment_post_ID)) {
                    ...
                    $post_title = ('' == $post_title) ? "# $comment->comment_post_ID" : $post_title; ?> | <a href="post.php?action=edit&amp;post=<?php echo $comment->comment_post_ID; ?>"><?php printf(__('Edit Post &#8220;%s&#8221;'), stripslashes($post_title)); ?></a>
                    <?php
                } ?>
	...
	<th scope="col" colspan="3">' . __('Actions') . '</th>
            ...
            foreach ($comments as $comment) {
                ...
                <tr class='<?php echo $class; ?>'>
                    ...
                    echo "<a href='post.php?action=editcomment&amp;comment=$comment->comment_ID' class='edit'>" . __('Edit') . '</a>';
                } ?></td>
                    <td><?php if (current_user_can('edit_post', $comment->comment_post_ID)) {
                    echo '<a href="' . wp_nonce_url('post.php?action=deletecomment&amp;p=' . $comment->comment_post_ID . '&amp;comment=' . $comment->comment_ID, 'delete-comment_' . $comment->comment_ID) . "\" onclick=\"return confirm('" . __(
                        "You are about to delete this comment.\\n  \'Cancel\' to stop, \'OK\' to delete."
