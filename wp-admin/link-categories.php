...
$wpvarstoreset = ['action', 'cat', 'auto_toggle'];
...
switch ($action) {
    case 'addcat':
    {
        ...
        $wpdb->query(
            "INSERT INTO $wpdb->linkcategories (cat_id, cat_name, auto_toggle, show_images, show_description, \n"
    ...
    case 'Edit':
    {
        ...
        $cat_id = (int)$_GET['cat_id'];
        $row = $wpdb->get_row(
            'SELECT cat_id, cat_name, auto_toggle, show_images, show_description, ' . ' show_rating, show_updated, sort_order, sort_desc, text_before_link, text_after_link, ' . " text_after_all, list_limit FROM $wpdb->linkcategories WHERE cat_id=$cat_id"
        );
        if ($row) {
            if (-1 == $row->list_limit) {
                $row->list_limit = '';
                            ...
                            <tr>
                                ...
                                <td>
                                    <select name="sort_order" size="1">
                                        <option value="name" <?php echo ('name' == $row->sort_order) ? 'selected="selected"' : '' ?>><?php _e('Name') ?></option>
                                        <option value="id" <?php echo ('id' == $row->sort_order) ? 'selected' : '' ?>><?php _e('Id') ?></option>
                                        <option value="url" <?php echo ('url' == $row->sort_order) ? 'selected' : '' ?>><?php _e('URL') ?></option>
                                        <option value="rating" <?php echo ('rating' == $row->sort_order) ? 'selected' : '' ?>><?php _e('Rating') ?></option>
                                        <option value="updated" <?php echo ('updated' == $row->sort_order) ? 'selected' : '' ?>><?php _e('Updated') ?></option>
                                        <option value="rand" <?php echo ('rand' == $row->sort_order) ? 'selected' : '' ?>><?php _e('Random') ?></option>
                                        <option value="length" <?php echo ('length' == $row->sort_order) ? 'selected' : '' ?>><?php _e('Name Length') ?></option>
                                    </select>
                                    <label>
    ...
    case 'editedcat':
    {
        ...
        if (isset($submit)) {
            ...
            $wpdb->query(
                "UPDATE $wpdb->linkcategories set
            cat_name='$cat_name',
    ...
    default:
    {
        ...
        <div class="wrap">
            ...
            'SELECT cat_id, cat_name, auto_toggle, show_images, show_description, ' . ' show_rating, show_updated, sort_order, sort_desc, text_before_link, text_after_link, ' . " text_after_all, list_limit FROM $wpdb->linkcategories ORDER BY cat_id"
        );
                $i = 1;
                foreach ((array)$results as $row) {
                    ...
                    */
                    ...
                    <tr id="link-category-<?php echo $row->cat_id; ?>" valign="middle" align="center" <?php echo $style ?> style="border-bottom: 1px dotted #9C9A9C;">
                        ...
                        <td><?php echo 'Y' == $row->auto_toggle ? __('Yes') : __('No') ?></td>
                        <td><?php echo 'Y' == $row->show_images ? __('Yes') : __('No') ?></td>
                        <td><?php echo 'Y' == $row->show_description ? __('Yes') : __('No') ?></td>
                        <td><?php echo 'Y' == $row->show_rating ? __('Yes') : __('No') ?></td>
                        <td><?php echo 'Y' == $row->show_updated ? __('Yes') : __('No') ?></td>
                        <td><?php echo $row->sort_order ?></td>
                        <td><?php echo 'Y' == $row->sort_desc ? __('Yes') : __('No') ?></td>
                        ...
                        _e('Default');
                            ...
                            <a href="<?php echo wp_nonce_url("link-categories.php?cat_id=$row->cat_id?>&amp;action=Delete", 'delete-link-category_' . $row->cat_id) ?>"
                               onclick="return deleteSomething( 'link category', <?php echo $row->cat_id . ", '" . sprintf(__('You are about to delete the &quot;%s&quot; link category.\\n&quot;Cancel&quot; to stop, &quot;OK&quot; to delete.'), js_escape($row->cat_name)); ?>' );"
                               class="delete"><?php _e('Delete') ?></a></td>
                        <?php } ?>
                    </tr>
                    <?php
                    ++$i;
