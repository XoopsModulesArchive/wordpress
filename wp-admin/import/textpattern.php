...
class Textpattern_Import
{
    ...
    public function get_txp_users()
    {
        ...
        return $txpdb->get_results(
            'SELECT
										...
										email,
										privs
							   		FROM ' . $prefix . 'txp_users',
            ARRAY_A
        );
    }

    public function get_txp_posts()
    {
        // General Housekeeping
    ...
    public function get_txp_comments()
    {
        global $wpdb;
        ...
        if (is_array($categories)) {
            ...
            foreach ($categories as $category) {
                ...
                if ($cinfo = category_exists($name)) {
                    $ret_id = wp_insert_category(['cat_ID' => $cinfo, 'category_nicename' => $name, 'cat_name' => $title]);
                } else {
                    $ret_id = wp_insert_category(['category_nicename' => $name, 'cat_name' => $title]);
                }
    ...
    public function users2wp($users = '')
    {
        ...
        if (is_array($users)) {
            ...
            foreach ($users as $user) {
                ...
                $transperms = [1 => '10', 2 => '9', 3 => '5', 4 => '4', 5 => '3', 6 => '2', 7 => '0'];
    ...
    public function posts2wp($posts = '')
    {
        ...
        if (is_array($posts)) {
            ...
            foreach ($posts as $post) {
                ...
                $stattrans = [1 => 'draft', 2 => 'private', 3 => 'draft', 4 => 'publish', 5 => 'publish'];

                //Can we do this more efficiently?

                $uinfo = (get_userdatabylogin($AuthorID)) ?: 1;
                ...
                if ($cat1 = get_catbynicename($Category1)) {
                    $cats[1] = $cat1;
                }

                if ($cat2 = get_catbynicename($Category2)) {
                    $cats[2] = $cat2;
                }
    ...
    public function cleanup_txpimport()
    {
        delete_option('tpre');
...
register_importer('textpattern', __('Textpattern'), __('Import posts from a Textpattern Blog'), [$txp_import, 'dispatch']);
