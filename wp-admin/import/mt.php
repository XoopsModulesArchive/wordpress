...
class MT_Import
{
    ...
    public function users_form($n)
    {
        global $wpdb, $testing;

        $users = $wpdb->get_results("SELECT * FROM $wpdb->users ORDER BY ID"); ?><select name="userselect[<?php echo $n; ?>]">
    ...
    public function checkauthor($author)
    {
        ...
        if (!(in_array($author, $this->mtnames, true))) { //a new mt author name is found
            ++$this->j;

            $this->mtnames[$this->j] = $author; //add that new mt author name to an array
        $user_id = username_exists($this->newauthornames[$this->j]); //check if the new author name defined by the user is a pre-existing wp user
        if (!$user_id) { //banging my head against the desk now.
            if ('left_blank' == $newauthornames[$this->j]) { //check if the user does not want to change the authorname
            ...
            $key = array_search($author, $this->mtnames, true); //find the array key for $author in the $mtnames array
    ...
    public function get_mt_authors()
    {
        $temp = [];
        ...
        foreach ($this->posts as $post) {
            if ('' != trim($post)) {
                ...
                $temp[] = (string)$thematch; //store the extracted author names in a temporary array
            }
        }
    ...
    public function get_authors_from_post()
    {
        $formnames = [];
        ...
        foreach ($_POST['user'] as $key => $line) {
            ...
            $formnames[] = (string)$newname;
        } // $formnames is the array with the form entered names

        foreach ($_POST['userselect'] as $user => $key) {
            $selected = trim(stripslashes($key));

            $selectnames[] = (string)$selected;
        }

        $count = count($formnames);

        for ($i = 0; $i < $count; $i++) {
            if ('#NONE#' != $selectnames[$i]) { //if no name was selected from the select menu, use the name entered in the form
                $this->newauthornames[] = (string)$selectnames[$i];
            } else {
                $this->newauthornames[] = (string)$formnames[$i];
            }
        }
    }

    public function mt_authors_form()
    {
        ?>
<div class="wrap">
    ...
    public function select_authors()
    {
                ...
                foreach ($metadata as $line) {
                    ...
                    switch ($key) {
                        ...
                        case 'DATE':
                            ...
                            $post_modified_gmt = get_gmt_from_date((string)$post_modified);
    ...
    register_importer('mt', __('Movable Type and Typepad'), __('Imports <strong>posts and comments</strong> from your Movable Type or Typepad blog'), [$mt_import, 'dispatch']);
    ?>
