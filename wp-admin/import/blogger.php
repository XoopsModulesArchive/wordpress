...
class Blogger_Import
{
    ...
    public function refresher($msec)
    {
        if ($msec) {
    ...
    public function parse_response($this_response)
    {
        // Split response into header and body sections

        [$response_headers, $response_body] = explode("\r\n\r\n", $this_response, 2);
        ...
        foreach ($cookies as $this_cookie) {
            $cookie_array[] = 'Cookie: ' . $this_cookie;
        }

        return ['code' => $response_code, 'header' => $response_header_array, 'cookies' => $cookie_array, 'body' => $response_body];
    }

    // Prints a form for the user to enter Blogger creds.

    public function login_form($text = '')
    {
        echo '<h1>' . __('Log in to Blogger') . "</h1>\n$text\n";

        echo '<form method="post" action="admin.php?import=blogger&amp;noheader=true&amp;step=0"><table><tr><td>'
             . __('Username')
             . ':</td><td><input type="text" name="user"></td></tr><tr><td>'
             . __('Password')
             . ':</td><td><input type="password" name="pass"></td><td><input type="submit" value="'
             . __('Start')
             . '"></td></tr></table></form>';
    ...
    public function login_blogger($user, $pass)
    {
    ...
    public function get_blogger($url, $header = '', $user = false, $pass = false)
    {
        $ch = curl_init();
    ...
    public function post_blogger($url, $header = false, $paramary = false, $parse = true)
    {
        $params = '';
    ...
    public function show_blogs()
    {
        ...
        foreach ($this->import['blogs'] as $blog) {
            if (9 == $blog['nextstep']) {
                $status = '100%';
            } elseif (8 == $blog['nextstep']) {
                $status = '90%';
            } elseif (7 == $blog['nextstep']) {
                $status = '82.5%';
            } elseif (6 == $blog['nextstep']) {
                $status = '75%';
            } elseif (5 == $blog['nextstep']) {
                $status = '57%';
            } elseif (4 == $blog['nextstep']) {
                $status = '28%';
            } elseif (3 == $blog['nextstep']) {
                $status = '14%';
            } else {
                $status = '0%';
            }
    ...
    public function publish_blogger($i, $text)
    {
        $head = $this->refresher(2000) . "<h1>$text</h1>\n";

        if (!mb_strstr($this->import['blogs'][$_GET['blog']]['publish'][$i], 'http')) {
            ...
            $paramary = ['blogID' => $_GET['blog'], 'all' => '1', 'republishAll' => 'Republish Entire Blog', 'publish' => '1', 'redirectUrl' => "/publish.do?blogID={$_GET['blog']}&inprogress=true"];
    ...
    public function set_next_step($step)
    {
    ...
    public function do_login()
    {
        if ((!$this->import['user'] && !is_array($this->import['cookies']))) {
            ...
            if (!($_POST['user'] && $_POST['pass'])) {
                $this->login_form(
                    __('The script will log into your Blogger account, change some settings so it can read your blog, and restore the original settings when it\'s done. Here\'s what you do:') . '</p><ol><li>' . __('Back up your Blogger template.') . '</li><li>' . __(
                        'Back up any other Blogger settings you might need later.'
                    ) . '</li><li>' . __('Log out of Blogger') . '</li><li>' . __('Log in <em>here</em> with your Blogger username and password.') . '</li><li>' . __('On the next screen, click one of your Blogger blogs.') . '</li><li>' . __(
            ...
            if (!mb_stristr($response['body'], 'signed in as')) {
                $this->login_form(__('Login failed. Please re-enter your username and password.'));
            }
            ...
            $template = '<MainPage><br><br><br><p>'
                                     . __('Are you looking for %title%? It is temporarily out of service. Please try again in a few minutes. Meanwhile, discover <a href="http://wordpress.org/">a better blogging tool</a>.')
            ...
            foreach ($blogsary[1] as $key => $id) {
                ...
                $blog_opts = [
                    ...
                    'blog-options-archiving' => ['archiveFrequency' => 'm'],
                    'blog-publishing' => ['publishMode' => '0', 'blogID' => (string)$id, 'subdomain' => mt_rand() . mt_rand(), 'pingWeblogs' => 'false'],
                    'blog-formatting' => ['timeStampFormat' => '0', 'encoding' => 'UTF-8', 'convertLineBreaks' => 'false', 'floatAlignment' => 'false'],
                    'blog-comments' => ['commentsTimeStampFormat' => '0'],
                    'template-edit' => ['templateText' => str_replace('%title%', trim($blogsary[2][$key]), $template)],
                ];

                // Build the blog options array template

                foreach ($blog_opts as $blog_opt => $modify) {
                    $new_opts[(string)$blog_opt] = ['backup' => false, 'modify' => $modify, 'error' => false];
                }

                $this->import['blogs'][(string)$id] = [
                    'id' => $id,
    ...
    public function backup_settings()
    {
        ...
        foreach ($this->import['blogs'][$_GET['blog']]['options'] as $blog_opt => $optary) {
            if ($blog_opt == $_GET['form']) {
                ...
                $this->import['blogs'][$_GET['blog']]['options'][(string)$blog_opt]['backup'] = $_POST;
                ...
                if ($optary['modify']) {
                    ...
                    $headers = array_merge($this->import['blogs'][$_GET['blog']]['options'][(string)$blog_opt]['cookies'], $this->import['cookies']);
                    ...
                    if ($response['code'] >= 400 || mb_strstr($response['body'], 'There are errors on this form')) {
                        die('<p>' . __('Error on form submission. Retry or reset the importer.') . '</p>' . addslashes(print_r($response, 1)));
                    }
                }

                $output .= "<del><p>$blog_opt</p></del>\n";
            } elseif (is_array($this->import['blogs'][$_GET['blog']]['options'][(string)$blog_opt]['backup'])) {
                ...
                $this->import['blogs'][$_GET['blog']]['options'][(string)$blog_opt]['cookies'] = $response['cookies'];
    ...
    public function get_archive_urls()
    {
        $bloghtml = $this->get_blogger($this->import['blogs'][$_GET['blog']]['url']);

        if (!mb_strstr($bloghtml['body'], '<a class="archive"')) {
    ...
    public function get_archive()
    {
        ...
        foreach ($this->import['blogs'][$_GET['blog']]['archives'] as $url => $status) {
            $archivename = mb_substr(basename($url), 0, 7);
            ...
            // Do nothing.
                ...
                $this->import['blogs'][$_GET['blog']]['archives'][(string)$url] = $status;
                ...
                for ($i = 1, $iMax = count($posts); $i < $iMax; $i += 1) {
                    ...
                    $post_author_email = $postinfo[5] ?: 'user@wordpress.org';
                    ...
                    if ($this->lump_authors) {
                        ...
                        if (!username_exists($post_author_name)) {
                            ...
                            $user_password = mb_substr(md5(uniqid(microtime())), 0, 6);
                    ...
                    if (('PM' == $post_date[2]) && ('12' != $posthour)) {
                        $posthour += 12;
                    } elseif (('AM' == $post_date[2]) && ('12' == $posthour)) {
                        $posthour = '00';
                    }
                    ...
                    $post_content = str_replace(['<br>', '<BR>', '<br>', '<br>', '<br>', '<BR>'], "\n", $post_content); // the XHTML touch... ;)
                    ...
                    if ($postparts[1]) :
                        for ($j = 1, $jMax = count($postparts); $j < $jMax; $j += 1) {
                            ...
                            if (('PM' == $comment_date[2]) && ('12' != $commenthour)) {
                                $commenthour += 12;
                            } elseif (('AM' == $comment_date[2]) && ('12' == $commenthour)) {
                                $commenthour = '00';
                            }
                            ...
                            if (mb_strpos($commentinfo[1], 'a href')) {
                                $comment_author_parts = explode('&quot;', htmlentities($commentinfo[1], ENT_QUOTES | ENT_HTML5));
                            ...
                            $comment_content = str_replace(['<br>', '<BR>', '<br>', '<br>', '<br>', '<BR>'], "\n", $comment_content);
                ...
                $status = sprintf(__('%s post(s) parsed, %s skipped...'), $postcount, $skippedpostcount) . ' ' . sprintf(__('%s comment(s) parsed, %s skipped...'), $commentcount, $skippedcommentcount) . ' ' . ' <strong>' . __('Done') . '</strong>';

                $import = $this->import;

                $import['blogs'][$_GET['blog']]['archives'][(string)$url] = $status;
        ...
        foreach ($this->import['blogs'][$_GET['blog']]['options'] as $blog_opt => $optary) {
            if ($did_one) {
                ...
                if ($optary['backup'] != $optary['modify']) {
                    ...
                    if ($response['code'] >= 400 || mb_strstr($response['body'], 'There are errors on this form')) {
                        $this->import['blogs'][$_GET['blog']]['options'][(string)$blog_opt]['error'] = true;

                        update_option('import-blogger', $this->import);

                        $output .= sprintf(__('%s failed. Trying again.'), "<p><strong>$blog_opt</strong> ") . '</p>';
                    } else {
                        $this->import['blogs'][$_GET['blog']]['options'][(string)$blog_opt]['restored'] = true;
    ...
    public function republish_blog()
    {
...
register_importer('blogger', __('Blogger and Blogspot'), __('Import <strong>posts and comments</strong> from your Blogger account'), [$blogger_import, 'start']);
