...
function valid_utf8($string)
{
    ...
    while ($i < $len) {
        ...
        if (valid_1byte($char)) {    // continue
            ...
            if (!valid_nextbyte(ord(mb_substr($string, $i++, 1)))) {
                return false;
            }
...
class Dotclear_Import
{
    ...
    public function get_dc_posts()
    {
        ...
        return $dcdb->get_results(
            'SELECT dc_post.*, dc_categorie.cat_libelle_url AS post_cat_name
						FROM dc_post INNER JOIN dc_categorie
						  ON dc_post.cat_id = dc_categorie.cat_id',
            ARRAY_A
        );
    }

    public function get_dc_comments()
    {
        global $wpdb;
    ...
    public function cat2wp($categories = '')
    {
        // General Housekeeping
        ...
        if (is_array($categories)) {
            ...
            foreach ($categories as $category) {
                ...
                if ($cinfo = category_exists($name)) {
                    $ret_id = wp_insert_category(['cat_ID' => $cinfo, 'category_nicename' => $name, 'cat_name' => $title, 'category_description' => $desc]);
                } else {
                    $ret_id = wp_insert_category(['category_nicename' => $name, 'cat_name' => $title, 'category_description' => $desc]);
                }
    ...
    public function users2wp($users = '')
    {
        // General Housekeeping
    ...
    public function posts2wp($posts = '')
    {
        ...
        if (is_array($posts)) {
            ...
            foreach ($posts as $post) {
                ...
                $stattrans = [0 => 'draft', 1 => 'publish'];

                $comment_status_map = [0 => 'closed', 1 => 'open'];

                //Can we do this more efficiently?

                $uinfo = (get_userdatabylogin($user_id)) ?: 1;
                ...
                if ($cat1 = get_catbynicename($post_cat_name)) {
                    $cats[1] = $cat1;
                }
    ...
    public function comments2wp($comments = '')
    {
        // General Housekeeping
        ...
        if (is_array($comments)) {
            ...
            foreach ($comments as $comment) {
                ...
                $comment_approved = (string)$comment_pub;
    ...
    public function links2wp($links = '')
    {
        // General Housekeeping
            ...
            foreach ($links as $link) {
                ...
                if ('' != $title) {
                    if ($cinfo = link_cat_exists(csc($title))) {
                        ...
                        $wpdb->query(
                            "INSERT INTO $wpdb->linkcategories (cat_name) VALUES ('" . $wpdb->escape(csc($title)) . "')"
                        );
    ...
    public function cleanup_dcimport()
    {
        delete_option('tpre');
...
register_importer('dotclear', __('Dotclear'), __('Import posts from a Dotclear Blog'), [$dc_import, 'dispatch']);
