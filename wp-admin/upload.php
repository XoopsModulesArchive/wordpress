    ...
    <form action="upload.php" method="post" enctype="multipart/form-data">
        ...
        <p>
            <label for="thumbsize_no">
                <input type="radio" name="thumbsize" value="none" checked id="thumbsize_no">
...
case 'upload':
    ...
    $imgalt = $_POST['imgalt'] ?? $imgalt;

    $img1_name = (mb_strlen($imgalt)) ? $_POST['imgalt'] : $_FILES['img1']['name'];
    $img1_type = (mb_strlen($imgalt)) ? $_POST['img1_type'] : $_FILES['img1']['type'];
    $imgdesc = str_replace('"', '&amp;quot;', $_POST['imgdesc']);

    $imgtype = explode('.', $img1_name);
    $imgtype = $imgtype[count($imgtype) - 1];

    if (false === in_array($imgtype, $allowed_types, true)) {
    ...
    while (file_exists($pathtofile2)) {
        $pos = mb_strpos($tmppathtofile, '.' . trim($imgtype));

        $pathtofile_start = mb_substr($tmppathtofile, 0, $pos);
    ...
    if (file_exists($pathtofile) && !mb_strlen($imgalt)) {
    ...
    if (!mb_strlen($imgalt)) {
        ...
        rename($img1, $pathtofile) || die("Couldn't Upload Your File to $pathtofile.");
    }

    if (('none' != $_POST['thumbsize']) && ('icon' != $_POST['thumbsize'])) {
        if ('small' == $_POST['thumbsize']) {
            $max_side = 200;
        } elseif ('large' == $_POST['thumbsize']) {
            $max_side = 400;
        } elseif ('custom' == $_POST['thumbsize']) {
    ...
    $img_prefix = 'thumb-';
    if ('amazon' == $_POST['associate']) {
        $asin = explode('.', $img1_name);

        $piece_of_code = "&lt;a href=&quot;http://www.amazon.co.jp/exec/obidos/ASIN/$asin[0]/$amazon_id&quot; target=&quot;_blank&quot;&gt;&lt;img style=&quot;float: left; margin: 0 10px 0 0;&quot; src=&quot;"
                         . get_settings('fileupload_url')
                         . "/$img1_name&quot; border=&quot;0&quot; alt=&quot;$imgdesc&quot; /&gt;&lt;/a&gt;";
    } elseif ('icon' == $_POST['thumbsize']) {
    ...
    } elseif (preg_match('image/', $img1_type) && 'none' != $_POST['thumbsize']) {
