...
$wpvarstoreset = ['action', 'safe_mode', 'withcomments', 'posts', 'content', 'edited_post_title', 'comment_error', 'profile', 'trackback_url', 'excerpt', 'showcomments', 'commentstart', 'commentend', 'commentorder'];
...
switch ($action) {
    ...
    case 'edit':
        ...
        if ('static' == $post->post_status) {
            ...
            require __DIR__ . '/edit-form-advanced.php';
        }
    ...
    case 'delete':
        ...
        if (mb_strstr($sendback, 'post.php')) {
            $sendback = get_settings('siteurl') . '/wp-admin/post.php';
        } elseif (mb_strstr($sendback, 'attachments.php')) {
            $sendback = get_settings('siteurl') . '/wp-admin/attachments.php';
        }
    ...
    case 'editcomment':
        ...
        require __DIR__ . '/edit-form-comment.php';
    ...
    case 'deletecomment':
        ...
        $postdata = get_post($p) || die(sprintf(__('Oops, no post with this ID. <a href="%s">Go back</a>!'), 'edit.php'));
    ...
    case 'approvecomment':
        ...
        wp_set_comment_status($comment->comment_ID, 'approve');
        if (true === get_settings('comments_notify')) {
            wp_notify_postauthor($comment->comment_ID);
        }
...
<p>' . __('Right click on the following link and choose "Add to favorites" to create a posting shortcut.') . '</p>'; ?>
                <p>
                    ...
                    if ($is_NS4 || $is_gecko) {
                        ?>
                        <a href="javascript:if(navigator.userAgent.indexOf('Safari') >= 0){Q=getSelection();}else{Q=document.selection?document.selection.createRange().text:document.getSelection();}location.href='<?php echo get_settings(
                            'siteurl'
                        ) ?>/wp-admin/post.php?text='+encodeURIComponent(Q)+'&amp;popupurl='+encodeURIComponent(location.href)+'&amp;popuptitle='+encodeURIComponent(document.title);"><?php printf(__('Press It - %s'), wp_specialchars(get_settings('blogname'))); ?></a>
                    <?php
                    } elseif ($is_winIE) {
                        ?>
                        <a href="javascript:Q='';if(top.frames.length==0)Q=document.selection.createRange().text;location.href='<?php echo get_settings(
                            'siteurl'
                        ) ?>/wp-admin/post.php?text='+encodeURIComponent(Q)+'&amp;popupurl='+encodeURIComponent(location.href)+'&amp;popuptitle='+encodeURIComponent(document.title);"><?php printf(__('Press it - %s'), get_settings('blogname')); ?></a>
                    ...
                    <?php
                        ...
                        ?>
                        <a href="javascript:Q='';location.href='<?php echo get_settings('siteurl'); ?>/wp-admin/bookmarklet.php?text='+escape(document.getSelection())+'&popupurl='+escape(location.href)+'&popuptitle='+escape(document.title);"><?php printf(
                            __('Press it - %s'),
