    ...
    <div class="wrap">
        ...
        <div id="poststuff">
                ...
                <div id="grabit" class="dbx-group">
                    ...
                    <?php if (current_user_can('edit_posts')) : ?>
                        <fieldset id="posttimestampdiv" class="dbx-box">
                            ...
                            <div class="dbx-content"><?php touch_time(('edit' == $action)); ?></div>
                        </fieldset>
                    <?php endif; ?>
                            ...
                            <div class="dbx-content">
                                <select name="post_author_override" id="post_author_override">
                                    ...
                                    foreach ($authors as $o) :
                                        $o = get_userdata($o->ID);
                                        if ($post->post_author == $o->ID || (empty($post_ID) && $user_ID == $o->ID)) {
            ...
            <div><textarea <?php if (user_can_richedit()) {
                    echo 'title="true" ';
                } ?>rows="<?php echo $rows; ?>" cols="40" name="content" tabindex="2" id="content"><?php echo user_can_richedit() ? wp_richedit_pre($post->post_content) : $post->post_content; ?></textarea></div>
            </fieldset>
            ...
            <p class="submit"><?php echo $saveasdraft; ?> <input type="submit" name="submit" value="<?php _e('Save') ?>" style="font-weight: bold;" tabindex="4">
                ...
                } elseif (url_to_postid(wp_get_referer()) == $post_ID) {
                    echo 'redo';
                } else {
            ...
            <?php if ('edit' == $action) : $delete_nonce = wp_create_nonce('delete-post_' . $post_ID); ?>
                <input name="deletepost" class="button" type="submit" id="deletepost" tabindex="10" value="<?php _e('Delete this post') ?>" <?php echo "onclick=\"if ( confirm('" . sprintf(
                                    __("You are about to delete this post \'%s\'\\n  \'Cancel\' to stop, \'OK\' to delete."),
                                    js_escape($post->post_title)
                                ) . "') ) { document.forms.post._wpnonce.value = '$delete_nonce'; return true;}return false;\""; ?>>
            <?php endif; ?>
