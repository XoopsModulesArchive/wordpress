...
SELECT;
            ...
            $select = $wpdb->get_results(
                'SELECT optionvalue, optionvalue_desc ' . "FROM {$wpdb->optionvalues[$wp_id]} " . "WHERE option_id = $option_result->option_id " . 'ORDER BY optionvalue_seq'
            );
            if ($select) {
        ...
        case 7: // SQL select
            ...
            eval("require dirname(__DIR__) . '/wp-config.php');\$sql = \"$sql\";";
...
SELECT;
            ...
            $select = $wpdb->get_results((string)$sql);
            if ($select) {
                foreach ($select as $option) {
