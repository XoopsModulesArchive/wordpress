...
<?php
// Sub-menu
if (isset($submenu[(string)$parent_file])) :
    ?>
    <ul id="submenu">
        <?php
        foreach ($submenu[(string)$parent_file] as $item) :
            if (!current_user_can($item[1])) {
                continue;
            }
            ...
            } elseif ((isset($plugin_page) && $plugin_page == $item[2]) || (!isset($plugin_page) && $self == $item[2])) {
                $class = ' class="current"';
            } else {
                $class = '';
            }
