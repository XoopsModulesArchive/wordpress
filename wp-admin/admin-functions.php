...
function get_default_post_to_edit()
{
    ...
    } elseif (!empty($_REQUEST['popuptitle'])) {
...
function url_shorten($url)
{
    ...
    if ('/' == mb_substr($short_url, -1)) {
        $short_url = mb_substr($short_url, 0, -1);
    }

    if (mb_strlen($short_url) > 35) {
        $short_url = mb_substr($short_url, 0, 32) . '...';
    }
...
function get_nested_categories($default = 0, $parent = 0)
{
    ...
    if (is_array($cats)) {
        foreach ($cats as $cat) {
            ...
            $result[$cat]['checked'] = in_array($cat, $checked_categories, true);
...
function write_nested_categories($categories)
{
    foreach ($categories as $category) {
        echo '<label for="category-', $category['cat_ID'], '" class="selectit"><input value="', $category['cat_ID'], '" type="checkbox" name="post_category[]" id="category-', $category['cat_ID'], '"', ($category['checked'] ? ' checked' : ''), '> ', wp_specialchars(
            $category['cat_name']
        ), "</label>\n";
            ...
            if ($category->category_parent == $parent) {
                ...
                if (current_user_can('manage_categories')) {
                    ...
                    if ($category->cat_ID != $default_cat_id) {
                        $edit .= "<td><a href='" . wp_nonce_url("categories.php?action=delete&amp;cat_ID=$category->cat_ID", 'delete-category_' . $category->cat_ID) . "' onclick=\"return deleteSomething( 'cat', $category->cat_ID, '" . sprintf(
                            __('You are about to delete the category &quot;%s&quot;.  All of its posts will go to the default category.\\n&quot;OK&quot; to delete, &quot;Cancel&quot; to stop.'),
                            js_escape($category->cat_name)
        ...
        foreach ($pages as $post) {
            ...
            if ($post->post_parent == $parent) {
                ...
                <tr id='page-<?php echo $id; ?>' class='<?php echo $class; ?>'>
                    ...
                    <td><?php if (current_user_can('edit_pages')) {
                    echo "<a href='post.php?action=edit&amp;post=$id' class='edit'>" . __('Edit') . '</a>';
                } ?></td>
                    <td><?php if (current_user_can('edit_pages')) {
                    echo "<a href='" . wp_nonce_url("post.php?action=delete&amp;post=$id", 'delete-post_' . $id) . "' class='delete' onclick=\"return deleteSomething( 'page', " . $id . ", '" . sprintf(
                        __('You are about to delete the &quot;%s&quot; page.\\n&quot;OK&quot; to delete, &quot;Cancel&quot; to stop.'),
                        js_escape(get_the_title())
                    ) . "' );\">" . __('Delete') . '</a>';
                } ?></td>
                </tr>
...
function wp_create_thumbnail($file, $max_side, $effect = '')
{
    ...
    if (file_exists($file)) {
        ...
        if (!function_exists('imagegif') && 1 == $type[2]) {
            $error = __('Filetype not supported. Thumbnail not created.');
        } elseif (!function_exists('imagejpeg') && 2 == $type[2]) {
            $error = __('Filetype not supported. Thumbnail not created.');
        } elseif (!function_exists('imagepng') && 3 == $type[2]) {
            $error = __('Filetype not supported. Thumbnail not created.');
        } else {
...
<table id='meta-list' cellpadding="3">
        ...
        if (!file_exists($filename) || is_writable($filename)) {
            ...
            if ($markerdata) {
                ...
                foreach ($markerdata as $markerline) {
                    if (mb_strstr($markerline, "# BEGIN {$marker}")) {
                        $state = false;
                    }
                    ...
                    if (mb_strstr($markerline, "# END {$marker}")) {
    ...
    function the_quicktags()
    {
        ...
        if (!mb_strstr($_SERVER['HTTP_USER_AGENT'], 'Safari')) {
            echo '
		<div id="quicktags">
...
';
    ...
    function validate_current_theme()
    {
        ...
        if (('default' != $template) && (!file_exists("$theme_root/$template/index.php"))) {
        ...
        if (('default' != $stylesheet) && (!file_exists("$theme_root/$stylesheet/style.css"))) {
            update_option('template', 'default');
            ...
            foreach (array_keys($submenu) as $parent) {
                foreach ($submenu[$parent] as $submenu_array) {
                    if (isset($submenu_array[3])) {
                        ...
                        } elseif (isset($plugin_page) && ($plugin_page == $submenu_array[2]) && (($parent == $pagenow) || ($parent == $plugin_page) || ($plugin_page == $hook) || (('admin.php' == $pagenow) && ($parent1 != $submenu_array[2])))) {
                            $title = $submenu_array[3];
    ...
    function add_menu_page($page_title, $menu_title, $access_level, $file, $function = '')
    {
        ...
        $menu[] = [$menu_title, $access_level, $file, $page_title];
    ...
    function add_submenu_page($parent, $page_title, $menu_title, $access_level, $file, $function = '')
    {
        ...
        $submenu[$parent][] = [$menu_title, $access_level, $file, $page_title];
    ...
    function get_plugin_data($plugin_file)
    {
        ...
        return ['Name' => $name, 'Title' => $plugin, 'Description' => $description, 'Author' => $author, 'Version' => $version, 'Template' => $template[1]];
    }
    ...
    function get_plugins()
    {
        ...
        if ($plugins_dir) {
            while (false !== ($file = $plugins_dir->read())) {
                if (preg_match('|^\.+$|', $file)) {
                    continue;
                }
                ...
                if (is_dir($plugin_root . '/' . $file)) {
                    ...
                    if ($plugins_subdir) {
                        while (false !== ($subfile = $plugins_subdir->read())) {
    ...
    function get_plugin_page_hookname($plugin_page, $parent_page)
    {
        ...
        } elseif (isset($admin_page_hooks[$parent_page])) {
    ...
    // On failure, returns $overrides['upload_errorHandler'](&$file, $message) or array('error'=>$message).
    function wp_handle_upload(&$file, $overrides = false)
    {
        ...
        // You may define your own function and pass the name in $overrides['upload_errorHandler']

        $upload_errorHandler = 'wp_handle_upload_error';
        ...
        if ($test_form && (!isset($_POST['action']) || ($_POST['action'] != $action))) {
            return $upload_errorHandler($file, __('Invalid form submission.'));
        }

        // A successful upload will pass this test. It makes no sense to override this one.

        if ($file['error'] > 0) {
            return $upload_errorHandler($file, $upload_error_strings[$file['error']]);
        }

        // A non-empty file will pass this test.

        if ($test_size && !($file['size'] > 0)) {
            return $upload_errorHandler($file, __('File is empty. Please upload something more substantial.'));
        }

        // A properly uploaded file will pass this test. There should be no reason to override this one.

        if (!@is_uploaded_file($file['tmp_name'])) {
            return $upload_errorHandler($file, __('Specified file failed upload test.'));
        }
        ...
        if ($test_type) {
            ...
            if (!$type || !$ext) {
                return $upload_errorHandler($file, __('File type does not meet security guidelines. Try another.'));
            }
        }
        ...
        if (!(($uploads = wp_upload_dir()) && false === $uploads['error'])) {
            return $upload_errorHandler($file, $uploads['error']);
        }
        ...
        return ['file' => $new_file, 'url' => $url, 'type' => $type];
    }
    ...
    function wp_shrink_dimensions($width, $height, $wmax = 128, $hmax = 96)
    {
        if ($height <= $hmax && $width <= $wmax) {
            ...
            return [$wmax, (int)($height / $width * $wmax)];
        }
  

        return [(int)($width / $height * $hmax), $hmax];
    }
    ...
    function get_udims($width, $height)
    {
        if ($height <= 96 && $width <= 128) {
            ...
            return [128, (int)($height / $width * 128)];
        }
  

        return [(int)($width / $height * 96), 96];
    }
